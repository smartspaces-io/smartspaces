...
class JdomPrototypeProcessor {
  ...
  /**
   * Prototype name for the base prototype, which can be null to allow custom
   * base variables.
   */
  ...
  private void followPrototypeChain(Element element, List<Element> prototypeChain) {
    ...
    for (String prototypeName : prototypeNameList) {
      ...
      // The base prototype is a special case that is allowed to be used without
      // being defined. This allows it
      // to be a base prototype for everything, if desired, and will not cause
      // an error if not manually defined.
