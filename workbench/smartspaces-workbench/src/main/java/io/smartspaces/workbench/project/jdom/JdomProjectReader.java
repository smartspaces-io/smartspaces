...
package io.smartspaces.workbench.project.jdom;
...
import io.smartspaces.workbench.project.ProjectDeployment;
import io.smartspaces.workbench.project.ProjectReader;
import io.smartspaces.workbench.project.activity.ActivityProjectConstituent;
import io.smartspaces.workbench.project.constituent.AssemblyComponentProjectConstituent;
import io.smartspaces.workbench.project.constituent.BundleContentProjectConstituent;
...
public class JdomProjectReader extends JdomReader implements ProjectReader {
  ...
  /**
   * The configuration property that specifies which linking should be used for
   * project dependencies.
   */
  public static final String CONFIGURATION_NAME_PROJECT_DEPENDENCY_LINKING_DEFAULT =
      "smartspaces.workbench.dependency.linking.default";
  ...
  /**
   * Project definition file attribute name for the identifying name of a
   * dependency.
   */
  private static final String PROJECT_ATTRIBUTE_NAME_DEPENDENCY_ITEM_IDENTIFYING_NAME =
      "identifyingName";

  /**
   * Project definition file attribute name for the version range of a
   * dependency.
   */
  ...
  /**
   * Project definition file attribute default value for whether a dependency is
   * required.
   */
  private static final String PROJECT_ATTRIBUTE_VALUE_DEFAULT_DEPENDENCY_ITEM_REQUIRED =
      PROJECT_VALUE_TRUE;
   ...
   * dynamic.
   */
  private static final String PROJECT_ATTRIBUTE_VALUE_DEFAULT_DEPENDENCY_ITEM_DYNAMIC =
      PROJECT_VALUE_FALSE;
  ...
  /**
   * Project definition file attribute name for the value of a configuration
   * item.
   */
   ...
   */
  private static final ProjectDependencyLinking PROJECT_DEPENDENCY_LINKING_DEFAULT =
      ProjectDependencyLinking.RUNTIME;
   ...
   */
  public JdomProjectReader(SmartSpacesWorkbench workbench) {
    super(workbench);
  }
   ...
   */
  private void processPrototypeChain(Project project, Namespace projectNamespace,
   ...
   *          input element
   */
  private void configureProjectFromElement(Project project, Namespace projectNamespace,
      Element projectElement) {
    ...
    project.addExtraConstituents(getIndividualConstituent(projectNamespace,
        projectElement.getChild(ActivityProjectConstituent.ACTIVITY_ELEMENT, projectNamespace),
        project));
    project.addExtraConstituents(getIndividualConstituent(projectNamespace,
        projectElement.getChild(LibraryProjectConstituent.LIBRARY_ELEMENT, projectNamespace),
        project));
    project.addExtraConstituents(getIndividualConstituent(projectNamespace,
        projectElement.getChild(TasksProjectConstituent.ELEMENT_NAME_TASKS, projectNamespace),
        project));
  ...
  private void getMainData(Project project, Namespace projectNamespace, Element rootElement) {
    project.setName(getChildTextTrimmed(rootElement, projectNamespace, PROJECT_ELEMENT_NAME_NAME,
        project.getName()));
    project.setDescription(getChildTextTrimmed(rootElement, projectNamespace,
        PROJECT_ELEMENT_NAME_DESCRIPTION, project.getDescription()));
    ...
    String version =
        getChildTextTrimmed(rootElement, projectNamespace, PROJECT_ELEMENT_NAME_VERSION);
    if (version != null) {
      project.setVersion(Version.parseVersion(version));
    }

    String baseDirectoryPath =
        getChildTextTrimmed(rootElement, projectNamespace, PROJECT_ELEMENT_NAME_BASE_DIRECTORY);
  ...
  private void getConfiguration(Project project, Namespace projectNamespace, Element rootElement) {
    Element configurationElement =
        rootElement.getChild(PROJECT_ELEMENT_NAME_CONFIGURATION, projectNamespace);
   ...
   */
  private String getConfigurationValue(Namespace projectNamespace, Element propertyElement,
      String propertyName) {
  ...
  private void getDependencies(Project project, Namespace projectNamespace, Element rootElement) {
    Element dependenciesElement =
        rootElement.getChild(PROJECT_ELEMENT_NAME_DEPENDENCIES, projectNamespace);
  ...
  private ProjectDependencyLinking getDefaultProjectDependencyLinking() {
    return ProjectDependencyLinking.valueOf(getWorkbench().getWorkbenchConfig().getPropertyString(
        CONFIGURATION_NAME_PROJECT_DEPENDENCY_LINKING_DEFAULT,
        PROJECT_DEPENDENCY_LINKING_DEFAULT.toString()));
  }
   ...
   *
   * @return the linking value to use or {@code null} if the linkage value was
   *         not legal
   */
  ...
  private ProjectDependency getDependency(Element dependencyElement,
    ...
    if (versionStr != null) {
      ...
      addError("project.xml <dependency> has no version attribute");
      return null;
    }
    ...
    ProjectDependencyLinking linking =
        getDependencyLinkingAttribute(dependencyElement, defaultLinking);
    if (linking == null) {
      return null;
    }
  ...
  private void getDeployments(Project project, Namespace projectNamespace, Element rootElement) {
    Element deploymentsElement =
        rootElement.getChild(PROJECT_ELEMENT_NAME_DEPLOYMENTS, projectNamespace);
  ...
  private ProjectDeployment getDeployment(Element deploymentElement) {
    String type =
        getRequiredAttributeValue(deploymentElement, PROJECT_ATTRIBUTE_NAME_DEPLOYMENT_ITEM_TYPE);
    String method =
        getAttributeValue(deploymentElement, PROJECT_ATTRIBUTE_NAME_DEPLOYMENT_ITEM_METHOD);
    String location =
        getAttributeValue(deploymentElement, PROJECT_ATTRIBUTE_NAME_DEPLOYMENT_ITEM_LOCATION);
