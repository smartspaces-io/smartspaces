...
import io.smartspaces.liveactivity.runtime.standalone.messaging.MessageUtils.MessageSet;
import io.smartspaces.liveactivity.runtime.standalone.messaging.MessageUtils.MessageSetList;

import java.io.File;

/**
 * Object to manage checking produced messages to see if they match the
 * indicated check file.
 *
...
class MessageCheckRunner {
  ...
  public synchronized boolean checkMessage(MessageMap message) {
    ...
    if (messageIndex < expectedMessageList.size()) {
      if (nextExpectedMessage == null) {
        ...
        // Use the delay of the checked messages to figure out how long to wait.
        // This isn't precise, but
        // it should at least be a flexible way of waiting. Note that this is *2
        ...
        if (!(timeDelay instanceof Integer)) {
          throw new SimpleSmartSpacesException(String.format(
              "Message must have a valid Integer '%s' field: %s",
      ...
      if (MessageUtils.deepMatches(message, nextExpectedMessage)) {
        standaloneMessageRouter.getLog().info(
            String.format("Verified message index %d: %s", messageIndex,
                nextExpectedMessage.get(StandaloneMessageRouter.MESSAGE_KEY)));
        messageIndex++;
  ...
  /**
   * Verify completion. Will signal success or failure to containing activity
   * runner..
   */
  synchronized void finalizeVerification() {
    ...
    if (messageIndex < expectedMessageList.size()) {
      standaloneMessageRouter.getLog().error(
          String.format("Failed to verify message #%d: %s", messageIndex,
              expectedMessageList.get(messageIndex)));
