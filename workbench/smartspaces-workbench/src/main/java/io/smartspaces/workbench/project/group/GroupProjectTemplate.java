...
public class GroupProjectTemplate extends BaseProjectTemplate {
  ...
  public void onTemplateWrite(ProjectCreationContext context) {
    ...
    try {
      for (Project project : groupProjectTemplateSpecification.getProjectList()) {
        projectIndex++;
        context.getWorkbenchTaskContext().getWorkbench().getProjectCreator()
            .create(makeCreationSpecification(project, context));
      }
    } catch (Exception e) {
   ...
   */
  private ProjectCreationContext makeCreationSpecification(Project project,
      ProjectCreationContext groupCreationSpec) {
    ...
    creationSpecification.addTemplateDataEntry("baseDirectory",
        groupCreationSpec.getBaseDirectory());
    creationSpecification.addTemplateDataEntry("group",
        groupCreationSpec.getGroupProjectTemplateSpecification());
