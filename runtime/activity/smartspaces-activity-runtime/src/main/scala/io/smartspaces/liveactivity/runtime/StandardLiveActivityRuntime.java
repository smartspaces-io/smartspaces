...
public class StandardLiveActivityRuntime extends BaseActivityRuntime
  ...
  public void shutdown() {
    ...
    for (LiveActivityRuntimeListener listener : runtimeListeners) {
      try {
        ...
        getSpaceEnvironment().getLog().formatError("Error on live activity runtime onStartup call");
      }
    }
  }
  ...
  public void startupLiveActivity(String uuid) {
    try {
      ...
      if (liveActivityRunner != null) {
        ...
        getSpaceEnvironment().getLog().warn(String
      ...
      getSpaceEnvironment().getLog().formatError(e, "Error during startup of live activity: %s",
          uuid);
  ...
  public void shutdownLiveActivity(final String uuid) {
    try {
      ...
      if (liveActivityRunner != null) {
        getSpaceEnvironment().getLog().formatInfo("Shutting down activity: %s",
        ...
        if (liveActivity != null) {
          getSpaceEnvironment().getLog().formatInfo("Shutting down activity (wasn't running): %s",
          ...
          getSpaceEnvironment().getLog().formatError(
      ...
      getSpaceEnvironment().getLog().formatError(e, "Error during shutdown of live activity: %s",
          uuid);
  ...
  public void statusLiveActivity(String uuid) {
    LiveActivityRunner liveActivityRunner = getLiveActivityRunnerByUuid(uuid, false);
    if (liveActivityRunner != null) {
      getSpaceEnvironment().getLog().formatInfo("Getting status of live activity: %s",
          liveActivityRunner.getDisplayName());
      final ActivityStatus activityStatus = liveActivityRunner.sampleActivityStatus();
      getSpaceEnvironment().getLog().formatInfo("Reporting live activity status %s: %s",
  ...
  public void activateLiveActivity(String uuid) {
    try {
      ...
      LiveActivityRunner liveActivityRunner = getLiveActivityRunnerByUuid(uuid, true);
      if (liveActivityRunner != null) {
        getSpaceEnvironment().getLog().formatInfo("Activating live activity: %s",
            liveActivityRunner.getDisplayName());
      ...
      getSpaceEnvironment().getLog().formatError(e, "Error during activation of live activity: %s",
          uuid);
  ...
  public void deactivateLiveActivity(String uuid) {
    try {
      LiveActivityRunner liveActivityRunner = getLiveActivityRunnerByUuid(uuid, false);
      if (liveActivityRunner != null) {
        getSpaceEnvironment().getLog().formatInfo("Deactivating live activity: %s",
            liveActivityRunner.getDisplayName());
  ...
  public void configureLiveActivity(String uuid, Map<String, String> configuration) {
    try {
      LiveActivityRunner liveActivityRunner = getLiveActivityRunnerByUuid(uuid, true);
      if (liveActivityRunner != null) {
        getSpaceEnvironment().getLog().formatInfo("Configuring live activity: %s",
            liveActivityRunner.getDisplayName());
    ...
    try {
      ...
      if (liveActivityRunner != null) {
        if (!liveActivityRunner.getCachedActivityStatus().getState().isRunning()) {
          getSpaceEnvironment().getLog().formatInfo(
  ...
  public void initializeActivityInstance(InstalledLiveActivity installedActivity,
    ...
    instance.setName(
        configuration.getPropertyString(ActivityConfiguration.CONFIGURATION_NAME_ACTIVITY_NAME));
  ...
  @Override
  public ExtendedLog getActivityLog(InstalledLiveActivity installedActivity,
      Configuration configuration, ActivityFilesystem activityFilesystem) {
  ...
  @Override
  public void releaseActivityLog(ExtendedLog activityLog) {
  ...
  private void initializeActivityConfiguration(Configuration configuration, Activity activity) {
    ...
    configuration.setProperty(
        ActivityConfiguration.CONFIGURATION_NAME_ACTIVITY_FILESYSTEM_DIR_INSTALL,
  ...
  private void handleActivityListenerOnActivityStatusChange(final Activity activity,
    ...
    // object
    // pool.
    taskQueue.addTask(() -> handleActivityStateChange(activity, oldStatus, newStatus));
  }
  ...
  LiveActivityRunner getLiveActivityRunnerByUuid(String uuid, boolean create) {
    ...
    if (liveActivityRunner == null) {
      getSpaceEnvironment().getLog().formatWarn("Could not find live activity runner with uuid %s",
          uuid);
    }
  ...
  private void handleActivityStateChange(Activity activity, ActivityStatus oldStatus,
    ...
    if (error) {
      getSpaceEnvironment().getLog().formatError(newStatus.getException(),
          "Error for live activity %s, state is now %s", activity.getUuid(), newState);
    } else {
      getSpaceEnvironment().getLog().formatInfo("Live activity %s, state is now %s",
  ...
  private void attemptActivityStartup(LiveActivityRunner liveActivityRunner) {
    String uuid = liveActivityRunner.getUuid();
    getSpaceEnvironment().getLog().formatInfo("Attempting startup of activity %s", uuid);
  ...
  private void startupActivitySequence(final LiveActivityRunner liveActivityRunner,
    ...
    // object
    // pool.
    taskQueue.addTask(() -> activityStateTransitioners.transition(uuid,
  ...
  private boolean attemptActivityStateTransition(LiveActivityRunner liveActivityRunner,
    ...
    if (transitionResult == TransitionResult.OK) {
      ...
      getSpaceEnvironment().getLog().warn(String.format(noopMessage, liveActivityRunner.getUuid()));
  ...
  private void reportIllegalActivityStateTransition(LiveActivityRunner activity,
    ...
    getSpaceEnvironment().getLog().formatError("Tried to %s activity %s, was in state %s",
  ...
  private void handleActivityRemove(String uuid, RemoveActivityResult result) {
    getSpaceEnvironment().getLog().formatInfo("Removed live activity %s", uuid);
