...
import io.smartspaces.workbench.ui.wizard.JWizardDialog;
...
public class WorkbenchUi extends JFrame implements ActionListener {
   ...
   */
  private final WorkbenchSplitPane desktop;
  ...
  /**
   * Menu item for a Open Project.
   */
  private JMenuItem openMenuItem;

  /**
   * Menu item for Close Project.
   */
  private JMenuItem closeMenuItem;

  /**
   * Menu item for a Save.
   */
  private JMenuItem saveMenuItem;

  /**
   * Menu item for a Save All.
   */
  private JMenuItem saveAllMenuItem;

  /**
   * Menu item for a file revert.
   */
  ...
  /**
   * Menu item for exiting the workbench.
   */
  private JMenuItem exitMenuItem;

  /**
   * Menu item for a New item.
   */
  ...
  public JMenuBar getJMenuBar() {
    ...
    openMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_O, Event.CTRL_MASK, false));

    closeMenuItem = new JMenuItem("Close");
    fileMenu.add(closeMenuItem);
    closeMenuItem.addActionListener(this);
    closeMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_W, Event.CTRL_MASK, false));

    saveMenuItem = new JMenuItem("Save");
    fileMenu.add(saveMenuItem);
    saveMenuItem.addActionListener(this);
    saveMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, Event.CTRL_MASK, false));
    saveMenuItem.setEnabled(true);

    saveAllMenuItem = new JMenuItem("Save All");
    fileMenu.add(saveAllMenuItem);
    saveAllMenuItem.addActionListener(this);
    saveAllMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_S, Event.CTRL_MASK
    ...
    undoMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Z, InputEvent.CTRL_MASK, false));
    undoMenuItem.setEnabled(true);

    redoMenuItem = new JMenuItem("Redo");
    editMenu.add(redoMenuItem);
    redoMenuItem.addActionListener(this);
    redoMenuItem.setAccelerator(KeyStroke.getKeyStroke(KeyEvent.VK_Y, InputEvent.CTRL_MASK, false));
    ...
    cutMenuItem = new JMenuItem(defaultActionMap.get(DefaultEditorKit.cutAction));
    cutMenuItem.setText("Cut");
    editMenu.add(cutMenuItem);

    copyMenuItem = new JMenuItem(defaultActionMap.get(DefaultEditorKit.copyAction));
    cutMenuItem.setText("Copy");
    editMenu.add(copyMenuItem);

    pasteMenuItem = new JMenuItem(defaultActionMap.get(DefaultEditorKit.pasteAction));
    cutMenuItem.setText("Paste");
    editMenu.add(pasteMenuItem);

    editMenu.addSeparator();

    selectAllMenuItem = new JMenuItem(defaultActionMap.get(DefaultEditorKit.selectAllAction));
    ...
    activityStartupAction =
        new WorkbenchAction("Startup", null, "Startup the activity", new Integer(KeyEvent.VK_L)) {
          @Override
          public void actionPerformed(ActionEvent e) {
            workbench.getLog().info("Starting up activity");
          }
        };
    activityActivateAction =
        new WorkbenchAction("Activate", null, "Activate the activity", new Integer(KeyEvent.VK_L)) {
          @Override
          public void actionPerformed(ActionEvent e) {
            workbench.getLog().info("Activating activity");
          }
        };
    activityDeactivateAction =
        new WorkbenchAction("Deactivate", null, "Deactivate the activity", new Integer(
            KeyEvent.VK_L)) {
          @Override
          public void actionPerformed(ActionEvent e) {
    ...
    activityShutdownAction =
        new WorkbenchAction("Shutdown", null, "Shut the activity down", new Integer(KeyEvent.VK_L)) {
          @Override
          public void actionPerformed(ActionEvent e) {
            workbench.getLog().info("Shutting down activity");
          }
        };
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (source.equals(openMenuItem)) {
      ...
      JWizardDialog wizard =
          new JWizardDialog("New Project", this, new NewProjectWizard(this, workbench));
      ...
      workbench.doCommandsOnProject(currentProject,
          Lists.newArrayList(SmartSpacesWorkbench.COMMAND_BUILD));
  ...
  private void openActivityProject(File baseProjectDir) {
    ...
    if (workbench.getProjectManager().isProjectFolder(baseProjectDir)) {
      setCurrentProject(workbench.getProjectManager().readProject(baseProjectDir,
          workbench.getLog()));
    } else {
      JOptionPane.showMessageDialog(this, "The folder is not an Activity project folder",
   ...
   */
  public SmartSpacesWorkbench getWorkbench() {
    return workbench;
  }
  ...
  private abstract class WorkbenchAction extends AbstractAction {
     ...
     */
    public WorkbenchAction(String text, ImageIcon icon, String desc, Integer mnemonic) {
      super(text, icon);
