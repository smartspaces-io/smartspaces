...
public class Rectangle2 {
  ...
  public boolean contains(Rectangle2 rect) {
    return containsX(rect.x) && containsX(rect.x + rect.width) && containsY(rect.y)
        && containsY(rect.y + rect.height);
  }
  ...
  public boolean equal(Rectangle2 r, double fuzz) {
    return Math.abs(x - r.x) < fuzz && Math.abs(y - r.y) < fuzz
        && Math.abs(height - r.height) < fuzz && Math.abs(width - r.width) < fuzz;
  }
