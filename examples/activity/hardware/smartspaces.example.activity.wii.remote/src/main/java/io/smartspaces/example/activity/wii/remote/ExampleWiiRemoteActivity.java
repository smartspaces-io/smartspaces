...
public class ExampleWiiRemoteActivity extends BaseActivity {
   ...
   */
  public static final String CONFIGURATION_PROPERTY_WII_REMOTE_BLUETOOTH_ADDRESS =
      "activity.example.wii.remote.bluetooth.address";
  ...
  public void onActivitySetup() {
    String wiiRemoteBluetoothAddress =
    ...
    WiiRemoteEventListener listener = new WiiRemoteEventListener() {
      ...
      @Override
      public void onWiiRemoteButtonAccelerometerEvent(int button, double x, double y, double z) {
  ...
  /**
   * A Wii button has been pushed and there is accelerometer data. Do something
   * if activated.
   * 
   * @param button
   *          the button which was pushed
   ...
   */
  public void onWiiButtonAccelerometer(int button, double x, double y, double z) {
    if (isActivated()) {
      if (accelerometerSampler.sample()) {
        getLog().info(
            String.format("Wii button pressed %d and accelerometer %f, %f, %f", button, x, y, z));
