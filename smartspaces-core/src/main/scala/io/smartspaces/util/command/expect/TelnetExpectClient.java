...
public class TelnetExpectClient implements NetworkReachableExpectClient {
  ...
  public void connect() {
    try {
      ...
      throw new SmartSpacesException(String.format(
          "Error while connecting to telnet service at %s:%d", targetHost, targetPort), e);
    }
  }
  ...
  @Override
  public void expect(String expectedString) throws SmartSpacesException {
    checkStatus();

    try {
      expector.expect(expectedString);
    } catch (Exception e) {
      throw new SmartSpacesException(String.format("Error while expecting telnet command: %s",
          expectedString), e);
    }
  }
  ...
  private void checkStatus() {
    if (!loggedIn) {
      throw new SimpleSmartSpacesException("The telnet client is not logged in");
    }
  }
