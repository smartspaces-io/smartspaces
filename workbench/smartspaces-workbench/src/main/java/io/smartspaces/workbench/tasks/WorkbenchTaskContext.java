...
public class WorkbenchTaskContext {
   ...
   */
  public WorkbenchTaskContext(SmartSpacesWorkbench workbench, Configuration workbenchConfig) {
  ...
  private void logException(Throwable e) {
    if (e instanceof SimpleSmartSpacesException) {
      workbench.getLog().error(((SimpleSmartSpacesException) e).getCompoundMessage());
    } else {
  ...
  private void performMainTask(WorkbenchTask task) {
    try {
      ...
      if (!(e instanceof smartspacesWorkbenchTaskInterruptionException)) {
        errors = true;
        logException(e);
      }
    }
    ...
    File[] extraComponentFiles =
        fileSupport.newFile(
            fileSupport.newFile(workbench.getWorkbenchFileSystem().getInstallDirectory(),
                EXTRAS_BASE_FOLDER), extraComponent).listFiles(new FilenameFilter() {
          @Override
          public boolean accept(File dir, String name) {
   ...
   */
  private void processProjectPathSubDirectories(File projectPathBaseDir,
   ...
   */
  public Project
      getDynamicProjectFromProjectPath(String identifyingName, VersionRange versionRange) {
    // TODO(keith): Consider moving all project repository scanning
    // functionality into its own object that can be shared
    // between tasks.
    scanProjectPath();

    NamedVersionedResourceWithData<Project> project =
        projectsPath.getResource(identifyingName, versionRange);
    if (project != null) {
   ...
   * @author Keith M. Hughes
   */
  public static class smartspacesWorkbenchTaskInterruptionException extends
      SmartSpacesException {

    /**
     * Construct a new exception.
     */
    public smartspacesWorkbenchTaskInterruptionException() {
