...
final class Conversions {
  ...
  static {
    ...
    SIMPLE_TYPE_CONVERSIONS =
        ImmutableMap.<Class<?>, Conversion<?>>builder().put(Boolean.class, toBoolean)
            .put(Boolean.TYPE, toBoolean).put(Integer.class, toInt).put(Integer.TYPE, toInt)
            .put(Long.class, toLong).put(Long.TYPE, toLong).put(Float.class, toFloat)
            .put(Float.TYPE, toFloat).put(Double.class, toDouble).put(Double.TYPE, toDouble)
            .put(String.class, new IdentityConversion<String>(String.class))
            .put(Object.class, new IdentityConversion<Object>(Object.class)).build();
  }

  /**
   * Convert a given object to a given type. If conversion is impossible, throws
   * an exception.
   *
  ...
  public static <T> T convert(Type expectedGenericType, Class<T> expectedType, Object object) {
    ...
    return result != null ? Primitives.wrap(expectedType).cast(result) : cannotConvert(object,
        expectedType);
  }
  ...
  private static <T> T cannotConvert(Object input, Class<T> targetType) {
    if (input == null) {
      throw new NullPointerException(String.format("Cannot convert null to primitive type %s.",
          targetType.getName()));
    } else {
  ...
  /**
   * Represents a function that converts values to a proper generic type. For
   * example, if the target type is {@code Map&lt;String, MyInterface&gt;}, the
   * original map may contain maps that need to be transformed into an
   * {@link InterfaceMap}. If the target type is
   * {@code Map&lt;String, List&lt;MyInterface&gt;&gt;}, then the elements of
   * lists may need to be transformed.
   *
   * @param <T>
  ...
  private static final class RecursiveConversion<T> implements Function<Object, T> {
    ...
    public static <T> Function<Object, T> create(Type expectedGenericType, Class<T> expectedType) {
      if (SIMPLE_TYPE_CONVERSIONS.containsKey(expectedType) || expectedType.isInterface()) {
        ...
        throw new UnsupportedOperationException("Unsupported dynamic property type: "
            + expectedGenericType);
      }
    }
  ...
  /**
   * Represents an implicit conversion of a value from an original type to a
   * target type. If the value is {@code null} or if the original type is the
   * same as the target type, the original value will be returned without
   * transformations.
   */
  private abstract static class Conversion<T> implements Function<Object, T> {
     ...
     *
     * @return value of the target type, or {@code null} if the value cannot be
     *         converted
     */
  ...
  /**
   * A no-op conversion. If a given object is of a target type, it is returned
   * as-is. Otherwise, an exception is thrown.
   *
   * @param <T>
   *          target type
   */
  ...
  private static class NumberOrStringToDouble extends Conversion<Double> {
     ...
     */
    private static final Map<String, Double> SPECIAL_DOUBLE_VALUES = ImmutableMap
        .<String, Double>builder().put(Double.toString(Double.NaN), Double.NaN)
        .put(Double.toString(Double.NEGATIVE_INFINITY), Double.NEGATIVE_INFINITY)
        .put(Double.toString(Double.POSITIVE_INFINITY), Double.POSITIVE_INFINITY).build();
  ...
  private static class NumberOrStringToFloat extends Conversion<Float> {
     ...
     */
    private static final Map<String, Float> SPECIAL_FLOAT_VALUES = ImmutableMap
        .<String, Float>builder().put(Float.toString(Float.NaN), Float.NaN)
        .put(Float.toString(Float.NEGATIVE_INFINITY), Float.NEGATIVE_INFINITY)
        .put(Float.toString(Float.POSITIVE_INFINITY), Float.POSITIVE_INFINITY).build();
  ...
  /**
   * List conversion that recursively considers the list's type parameters. Raw
   * lists and wildcards are not supported.
   */
  private static class ListConversion {
    ...
    public Object convert(Type genericListType, Object list) {
      ...
      Function<Object, ?> transformation =
          RecursiveConversion.create(genericElementType, elementType);
      return Collections.unmodifiableList(Lists.transform((List<?>) list, transformation));
    }
  }

  /**
   * Map conversion that recursively considers the map's type parameters. Only
   * maps with String keys are supported. Raw maps and wildcards are not
   * supported.
   */
  private static class MapConversion {
    ...
    public Object convert(Type genericMapType, Object map) {
      ...
      if (genericKeyType != String.class) {
        throw new UnsupportedOperationException("Only maps with String keys are supported: "
            + genericMapType);
      }
      ...
      Function<Object, ?> transformation =
          RecursiveConversion.create(genericValueType, elementType);
