...
public abstract class BaseNativeApplicationRunner implements NativeApplicationRunner {
  ...
  /**
   * {@code true} if the process environment should be cleaned before the
   * process starts.
   */
  ...
  public void startup() {
    processLock.lock();
    try {
      if (!runnerState.compareAndSet(NativeApplicationRunnerState.NOT_STARTED,
          NativeApplicationRunnerState.STARTING)) {
  ...
  private Process attemptRun(boolean firstTime) throws SmartSpacesException {
    try {
      ...
      if (executableFolder != null) {
        ...
        log.info(String.format("Starting up native code in folder %s",
      ...
      if (firstTime) {
        ...
        throw SmartSpacesException.newFormattedException(e, "Can't start up native application "
            + executablePath);
      }
   ...
   */
  @VisibleForTesting
  void modifyEnvironment(Map<String, String> processEnvironment,
      Map<String, String> modificationEnvironment) {
  ...
  private void logProcessResultStreams() {
    try {
      ...
      log.error(
          String.format("Error reading native application process streams for %s",
      ...
      if (restarter.isRestarting()) {
        ...
        if (restartDuration > restartDurationMaximum) {
          log.error(String.format(
              "Native application would not restart. Maximum duration time %d passed.",
