...
/**
 * A base class for SmartSpacesFilesystem subclasses that validates
 * directories.
 *
 * @author Keith M. Hughes
 * @author Max Rebuschatis
 */
public class BasicSmartSpacesFilesystem implements SmartSpacesFilesystem,
    ManagedResource {
  /**
   ...
   */
  protected BasicSmartSpacesFilesystem(File baseInstallDirectory, File baseRuntimeDirectory) {
  ...
  private void checkWriteableDirectory(File dir, String type) {
    if (dir.exists()) {
      if (dir.isDirectory()) {
        ...
        if (!dir.canRead()) {
          throw new SimpleSmartSpacesException(String.format("The %s directory %s is not readable",
      ...
      if (!dir.mkdirs()) {
        throw new SimpleSmartSpacesException(String.format("Unable to create the %s directory %s",
            type, dir.getAbsolutePath()));
      }
    }
  }
  ...
  private void checkReadableDirectory(File dir, String type) {
    if (dir.exists()) {
      ...
      throw new SimpleSmartSpacesException(String.format("The %s directory %s does not exist",
          type, dir.getAbsolutePath()));
    }
  }
}
