...
import io.smartspaces.service.image.depth.internal.openni2.libraries.NiTE2Library.NiteUserTrackerHandle;
import io.smartspaces.service.image.depth.internal.openni2.libraries.NitePoint3f;
import io.smartspaces.service.image.depth.internal.openni2.libraries.NiteUserData;
import io.smartspaces.service.image.depth.internal.openni2.libraries.NiteUserTrackerFrame;
...
public class Openni2UserTrackerDepthCameraEndpoint implements UserTrackerDepthCameraEndpoint {
  ...
  public synchronized void startup() {
    ...
    IntValuedEnum<OniStatus> cameraStatus =
        OpenNI2Library.oniDeviceOpen(Pointer.pointerToCString(cameraId), camera);
  ...
  public synchronized void shutdown() {
    ...
    if (openniStatus != OniStatus.ONI_STATUS_OK) {
      log.error(OpenNi2Support.getFullOpenNIMessage("Could not shut down camera, status was %s",
          openniStatus));
    }
  ...
  private void processUserTrackFrame() {
    ...
    synchronized (this) {
      ...
      if (niteStatus != NiteStatus.NITE_STATUS_OK) {
        log.error(OpenNi2Support.getFullNiteMessage("Error while reading a user tracking frame",
            niteStatus));
        return;
      }
      ...
      try {
        ...
        niteStatus =
            NiTE2Library.niteUserTrackerFrameRelease(userTracker.get(), pointerTrackerFrame.get());
      }
    }
