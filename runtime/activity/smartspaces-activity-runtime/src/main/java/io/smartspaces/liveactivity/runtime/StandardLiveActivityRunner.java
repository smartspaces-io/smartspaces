...
public class StandardLiveActivityRunner implements LiveActivityRunner {
  ...
  public Activity getInstance() {
    if (obtainInstanceLock(InstanceLockState.OBTAIN)) {
      ...
      throw new SimpleSmartSpacesException("Could not obtain instance");
    }
  }
  ...
  public void setActivityStatus(ActivityStatus status) {
    if (obtainInstanceLock(InstanceLockState.STATUS)) {
      ...
      throw new SimpleSmartSpacesException(
          "Could not set activity status because could not get instance lock");
    }
  }
  ...
  /**
   * Set the activity status of the live activity, not protected by the instance
   * lock.
   *
   * @param status
