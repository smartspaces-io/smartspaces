...
package io.smartspaces.example.activity.comm.network.udp.client.data;
...
/**
 * A Smart Spaces Java-based activity that demonstrates using a UDP client that
 * transmits numeric data.
 *
 * <p>
 * The number of times the client has been activated will be sent as an int.
 * Deactivations send the negative of the activation count as an int.
 * Activations send {@code 3.14} as a float, deactivations send {@code 2.71} as
 * a float. The message sends the int first, followed by the float.
 *
 * <p>
 * The server responds with a float followed by an int. Server responses are
 * printed.
 *
...
public class DataUdpClientActivity extends BaseActivity {
  ...
  public void onActivitySetup() {
    ...
    int remoteUdpServerPort =
        getConfiguration().getRequiredPropertyInteger(CONFIGURATION_PROPERTY_UDP_SERVER_PORT);
  ...
  public void onActivityActivate() {
    udpClient.newWriteableUdpPacket(8).writeInt(++activationCount).writeFloat(3.14f)
        .write(udpServerAddress);
  }

  @Override
  public void onActivityDeactivate() {
    udpClient.newWriteableUdpPacket(8).writeInt(-activationCount).writeFloat(2.71f)
        .write(udpServerAddress);
  }
  ...
  private void handleUdpResponse(byte[] response, InetSocketAddress remoteAddress) {
    // The client must read the data in the exact order it is written by the
    // server.
    ...
    getLog().info(
        String.format("UDP client got response from server %s: %f %d", remoteAddress, floatNumber,
            intNumber));
  }
}
