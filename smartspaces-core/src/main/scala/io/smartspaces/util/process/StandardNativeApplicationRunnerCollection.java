...
public class StandardNativeApplicationRunnerCollection implements NativeApplicationRunnerCollection {
  ...
  @Override
  public NativeApplicationRunner
      addNativeApplicationRunner(NativeApplicationDescription description) {
    ...
    try {
      if (!runnerComplete.await(waitTime, TimeUnit.MILLISECONDS)) {
        SimpleSmartSpacesException.throwFormattedException(
            "The command %s did not complete in %d msec", description, waitTime);
      }
    } catch (SimpleSmartSpacesException e) {
      throw e;
    } catch (InterruptedException e) {
      SimpleSmartSpacesException.throwFormattedException("The command %s wait was interrupted",
          description);
    }
