...
public class AndroidJvmProjectExtension implements JvmProjectExtension {
  ...
  public void addToClasspath(List<File> classpath, ProjectTaskContext context) {
    ...
    if (androidJarFile.exists()) {
      ...
      throw new SimpleSmartSpacesException(String.format("Could not find Android jar file %s",
  ...
  public void postProcessJar(ProjectTaskContext context, File jarFile) {
    ...
    List<String> dxCommand =
        Arrays.asList(platformToolsDirectory + "dx", "--dex", "--output=classes.dex",
            jarFile.getAbsolutePath());

    List<String> aaptCommand =
        Arrays.asList(platformToolsDirectory + "aapt", "add", jarFile.getAbsolutePath(),
            "classes.dex");
