...
public abstract class MetadataEditFormSupport extends BaseSpaceMasterController {
  ...
  /**
   * Create a configuration form with the values of the metadata parameters in
   * it.
   *
   * @param metadata
  ...
  protected ConfigurationForm newMetadataForm(Map<String, Object> metadata) {
    ...
    if (metadata != null) {
      List<LabeledValue> configParameters = UiUtilities.getMetadataView(metadata);

      StringBuilder builder = new StringBuilder();
      for (LabeledValue parameter : configParameters) {
        builder.append(parameter.getLabel()).append("=").append(parameter.getValue()).append('\n');
      }
