...
public class JdomReader {
   ...
   */
  private static final String XML_PARSER_FEATURE_XINCLUDE =
   ...
   */
  public JdomReader(SmartSpacesWorkbench workbench) {
  ...
  Element getRootElement(File inputFile) {
    ...
    try {
      ...
      throw new SmartSpacesException(String.format("Exception while processing %s",
   ...
   */
  protected List<ProjectConstituent> getContainerConstituents(Namespace namespace,
      Element containerElement, Project project) {
   ...
   */
  protected List<ProjectConstituent> getIndividualConstituent(Namespace namespace,
      Element constituentElement, Project project) {
  ...
  private class MyEntityResolver implements EntityResolver2 {
    ...
    public InputSource resolveEntity(String publicId, String systemId) throws SAXException,
      ...
      throw SimpleSmartSpacesException.newFormattedException(
          "Unsupported call to %s.resolveEntity(%s, %s)", getClass().getCanonicalName(), publicId,
          systemId);
    }
    ...
    public InputSource getExternalSubset(String name, String baseUri) throws SAXException,
        IOException {
      throw SimpleSmartSpacesException.newFormattedException(
          "Unsupported call to %s.getExternalSubset(%s, %s)", getClass().getCanonicalName(), name,
          baseUri);
    }
    ...
    public InputSource resolveEntity(String name, String publicId, String baseUri, String systemId)
      ...
      try {
        ...
        String decodedSystemId =
            workbench.getSpaceEnvironment().getSystemConfiguration().evaluate(idExpression);

        File resolvedFile = null;
        if (!decodedSystemId.startsWith(HttpConstants.URL_PATH_COMPONENT_SEPARATOR)) {
          resolvedFile =
              fileSupport.newFile(fileSupport.getParentFile(fileSupport.newFile(new URI(baseUri))),
                  decodedSystemId);
        } else {
        ...
        if (!decodedSystemId.equals(systemId)) {
          getLog().info(
              String.format("XML entity %s (possibly xinclude) resolved to %s", idExpression,
                  resolvedFile.getAbsolutePath()));
        }
        return new InputSource(resolvedFile.toURI().toString());
      } catch (URISyntaxException e) {
        throw SmartSpacesException.newFormattedException(e, "Could not resolve XML entity");
      }
    }
  }
}
