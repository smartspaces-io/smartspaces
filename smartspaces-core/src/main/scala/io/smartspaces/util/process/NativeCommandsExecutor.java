...
public class NativeCommandsExecutor {
   ...
   */
  public void executeCommands(List<List<String>> commands) throws SmartSpacesException {
    for (List<String> command : commands) {
      NativeCommandRunner runner = new NativeCommandRunner();
      runner.execute(command);

      if (!runner.isSuccess()) {
        throw new SimpleSmartSpacesException(String.format("Command failed: %s", command));
      }
    }
  }
}
