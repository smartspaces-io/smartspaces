...
package io.smartspaces.service.audio.player.internal;
...
public class ScanningFileAudioRepository extends InMemoryAudioRepository {
  ...
  private void addMusicFile(File file) {
    if (!file.canRead()) {
      throw new SimpleSmartSpacesException(String.format("Cannot read music file %s",
  ...
  private void getMusicMetadata(File file, AudioTrackMetadata track) {
    try {
      ...
      throw new SmartSpacesException(String.format("Error while scanning audio file %s",
