...
package io.smartspaces.container.data;

import io.smartspaces.domain.basic.SpaceController;
import io.smartspaces.domain.support.DomainValidationResult;
import io.smartspaces.domain.support.DomainValidationResult.DomainValidationResultType;
import io.smartspaces.domain.support.SpaceControllerHostIdValidator;
import io.smartspaces.domain.support.SpaceControllerNameValidator;
import io.smartspaces.logging.ExtendedLog;

/**
 * A validator for space controller information for both a master and a space
 * controller.
 *
...
public class SpaceControllerInformationValidator {
   ...
   *
   * @return a string builder that will contain content if there are errors and
   *         empty otherwise
   */
  public StringBuilder checkControllerInfoForErrors(SpaceController spaceControllerInfo,
      ExtendedLog log) {
    ...
    if (nameValidate.getResultType() == DomainValidationResultType.ERRORS) {
      String error =
          String.format("Space controller has illegal name %s\n%s", name,
              nameValidate.getDescription());
      log.error(error);
