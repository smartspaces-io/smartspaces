...
public class NettyHttpRemoteRepositoryMasterWebServerHandler extends SimpleChannelUpstreamHandler {
   ...
   */
  private boolean handleWebRequest(ChannelHandlerContext context, HttpRequest req)
      throws IOException {
   ...
   *          the HTTP response
   */
  private void sendHttpResponse(ChannelHandlerContext context, HttpRequest request,
      HttpResponse response) {
    // Generate an error page if response status code is not OK (200).
    if (response.getStatus().getCode() != 200) {
      log.error(String.format("Error page for Repository server %s", request.getUri()));
      response.setContent(ChannelBuffers.copiedBuffer(response.getStatus().toString(),
          CharsetUtil.UTF_8));
