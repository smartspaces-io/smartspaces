...
package io.smartspaces.workbench.project.tasks;
...
import io.smartspaces.workbench.project.StandardProjectTaskNames;
...
public class TasksProjectConstituent extends BaseProjectConstituent {
  ...
  /**
   * The element attribute value for the task position being after the current
   * task.
   */
  public static final String ELEMENT_ATTRIBUTE_VALUE_TASK_POSITION_AFTER = "after";

  /**
   * The element attribute value for the task position being before the current
   * task.
   */
  ...
  public void processConstituent(Project project, ProjectContext context) {
    if (!(context instanceof ProjectTaskContext)) {
      SimpleSmartSpacesException.throwFormattedException(
          "Projects using context type %s cannot have tasks", context.getClass().getName());
      return;
    }
   ...
   * @author Keith M. Hughes
   */
  public static class TasksProjectConstituentBuilderFactory implements
  ...
  private static class ProjectTasksBuilder extends BaseProjectConstituentBuilder {
     ...
     */
    private void handleNativeTask(Namespace namespace, Project project,
        TasksProjectConstituent resource, Element taskElement, String name, String position) {
      ...
      String executablePath =
          nativeElement.getChildTextTrim(TASK_TYPE_NATIVE_EXECUTABLE_PATH, namespace);
      ...
      resource.addTaskFactory(name, position, new ProjectNativeWorkbenchTaskFactory(project,
          nativeDescription, templateSource, templateContent));
    }
  }
  ...
  public static class ProjectNativeWorkbenchTaskFactory extends NativeWorkbenchTaskFactory {
     ...
     */
    public ProjectNativeWorkbenchTaskFactory(Project project,
        NativeApplicationDescription nativeDescription, String templateSource,
    ...
    @Override
    protected void addAdditionalTemplateData(WorkbenchTaskContext workbenchTaskContext,
        Map<String, Object> data) {
      data.put(TEMPLATE_DATA_PROJECT, project);
      data.put(TEMPLATE_DATA_PROJECT_TASK_CONTEXT,
          workbenchTaskContext.getProjectTaskContext(project));
    }
  }
}
