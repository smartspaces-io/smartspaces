...
public class ExternalJavadocGenerator implements JavadocGenerator {
  ...
  /**
   * The amount of time to wait for the javadoc compiler to finish, in
   * milliseconds.
   */
  ...
  /**
   * Javadoc command arg for specifying the subpackages to be covered by
   * Javadoc.
   */
  public static final String COMMAND_ARG_SUBPACKAGES = "-subpackages";

  /**
   * Javadoc command arg for just specifying the source path for the javadoc
   * sources.
   */
  public static final String COMMAND_ARG_SOURCE_PATH = "-sourcepath";

  /**
   * Javadoc command arg for specifying where the generated Javadoc files should
   * go.
   */
  ...
  public void generate(ProjectTaskContext context) {
    ...
    File docBuildFolder = fileSupport.newFile(context.getRootBuildDirectory(), "docs/html/javadoc");
    fileSupport.directoryExists(docBuildFolder);
    fileSupport.deleteDirectoryContents(docBuildFolder);

    File classesFolder = fileSupport.newFile(context.getRootBuildDirectory(),
    ...
    description.addArguments(COMMAND_ARG_DOC_BUILD_FOLDER, docBuildFolder.getAbsolutePath(),
        COMMAND_ARG_SOURCE_PATH, sourcesFolder.getAbsolutePath(), COMMAND_ARG_SUBPACKAGES,
        getRootSubpackages(classesFolder), COMMAND_ARG_PUBLIC);

    workbenchTaskContext.getWorkbench().getLog()
        .info(String.format("Javadoc command is %s", description));
