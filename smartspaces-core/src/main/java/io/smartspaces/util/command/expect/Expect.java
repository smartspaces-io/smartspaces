...
public class Expect implements ManagedResource {
  ...
  public static Expect spawn(String command, ScheduledExecutorService executorService, Log log)
    ...
    try {
      ...
      throw new SmartSpacesException("Could not spawn expect process", e);
    }
  }
   ...
   */
  public Expect(InputStream input, OutputStream output, ScheduledExecutorService executorService,
      Log log) {
  ...
  public void startup() {
    ...
    try {
      ...
      close();
      throw new SmartSpacesException("Fatal error when initializing pipe or selector", e);
    }
   ...
   *           most unlikely
   */
  private Pipe.SourceChannel inputStreamToSelectableChannel(final InputStream input)
      throws IOException {
    Pipe pipe = Pipe.open();
    ...
    Runnable piping = new Runnable() {
      ...
      public void run() {
        ...
        try {
          ...
          log.warn("IOException when piping from InputStream, " + "now the piping thread will end",
              e);
        } finally {
  ...
  public int expect(long timeout, Object... patterns) throws Exception {
    ...
    for (Object o : patterns) {
      if (o instanceof String) {
        ...
        log.warn("Object " + o.toString() + " (class: " + o.getClass().getName()
            + ") is neither a String nor " + "a java.util.regex.Pattern, using as a literal String");
