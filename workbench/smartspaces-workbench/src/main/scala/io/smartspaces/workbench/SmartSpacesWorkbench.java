 ...
 */
public class SmartSpacesWorkbench {
   ...
   */
  public SmartSpacesWorkbench(SmartSpacesEnvironment spaceEnvironment,
  ...
  public void createOsgi(List<String> args) {
    ...
    for (int i = 0; i < args.size(); i++) {
      ...
      if (COMMAND_LINE_FLAG_OUTPUT.equals(arg)) {
        if (++i >= args.size()) {
          throw new SimpleSmartSpacesException("Missing output value from osgi command, use "
        ...
        if (++i >= args.size()) {
          throw new SimpleSmartSpacesException("Missing headers value from osgi command "
        ...
        if (++i >= args.size()) {
          throw new SimpleSmartSpacesException("Missing source dir value from osgi command "
              + COMMAND_LINE_FLAG_SOURCEDIR + " sourcedir");
        }

        workbenchTaskContext.addJarFiles(fileSupport.resolveFile(runDirectory, args.get(i)),
            sources);
      } else {
    ...
    if (sources.isEmpty()) {
      throw new SimpleSmartSpacesException("No input jar files for the osgi command");
    }
  ...
  public boolean doCommands(List<String> commands) {
    ...
    try {
      ...
      if (COMMAND_CREATE.equals(command)) {
        ...
        if (baseDir.isDirectory()) {
          if (projectManager.isProjectFolder(baseDir)) {
            ...
            log.warn(String.format("No commands to execute on the non-project directory %s",
                baseDir.getPath()));
          } else {
            ...
            if (COMMAND_RECURSIVE.equals(commandModifier)) {
              ...
              log.error(String.format("Cannot run command %s on non-project directory %s",
                  commandModifier, baseDir.getPath()));
              return false;
            }
          }
        } else {
      ...
      if (e instanceof SimpleSmartSpacesException) {
        log.error(((SimpleSmartSpacesException) e).getCompoundMessage());
      } else {
  ...
  private void createProject(List<String> commands) {
    ...
    JdomProjectGroupTemplateSpecificationReader projectReader =
        new JdomProjectGroupTemplateSpecificationReader(this);
    GroupProjectTemplateSpecification project =
        projectReader.readProjectGroupTemplateSpecification(specFile);

    ProjectCreationContext creationSpecification =
        new ProjectCreationContext(specFile.getAbsolutePath(), getLog());
      ...
      while (!commands.isEmpty()) {
        ...
        if (parts.length != 2) {
          SimpleSmartSpacesException.throwFormattedException(
              "Template argument 's' does not match pattern 'a=b'", templateParameter);
        }
  ...
  private String determineTemplateSpec(String type, String kind) {
    ...
    String parameterName = String.format("smartspaces.workbench.template.%s.%s", type, kind);
    String specPath = workbenchConfig.getPropertyString(parameterName);
    if (Strings.isNullOrEmpty(specPath)) {
      throw new SimpleSmartSpacesException("Missing spec path configuration " + parameterName);
    }
    return specPath;
  }
  ...
  private String removeArgument(List<String> commands, String description) {
    if (commands.isEmpty()) {
      throw new SimpleSmartSpacesException("Missing argument " + description);
    }
  ...
  public boolean doCommandsOnProject(Project project, List<String> commands) {
    ...
    ProjectTaskContext projectTaskContext =
        projectTaskManager.newProjectTaskContext(project, workbenchTaskContext);
   ...
   *          context for workbench tasks
   */
  public void addProjectIdeTasks(List<String> commands, Project project,
   ...
   *          context for workbench tasks
   */
  public void addProjectDeployTasks(List<String> commands, Project project,
      ProjectTaskContext projectTaskContext, WorkbenchTaskContext workbenchTaskContext) {
    String deploymentType = removeArgument(commands, "deployment type");
    projectTaskManager.addDeploymentTasks(deploymentType, project, projectTaskContext,
   ...
   */
  public SmartSpacesFilesystem getWorkbenchFileSystem() {
  ...
  public void handleError(String message, Throwable e) {
    throw new SimpleSmartSpacesException(message, e);
  }
