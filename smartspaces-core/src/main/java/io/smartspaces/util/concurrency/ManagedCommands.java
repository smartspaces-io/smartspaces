...
public interface ManagedCommands {
  ...
  /**
   * Executes a periodic command executes at the given frequency period between
   ...
   * same time if any of the command executions run longer than the period.
   *
   * <p>
   * The command will start immediately.
   *
   * <p>
   * If the command throws an exception, it will not be run again. The exception
   * will be logged.
   *
   * @param command
   *          the command to run
  ...
  /**
   * Executes a periodic command where the time between the termination of one
