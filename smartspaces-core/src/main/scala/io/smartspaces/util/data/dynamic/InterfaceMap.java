...
/**
 * Dynamic object factory. Given an interface and a map, creates an object with
 * that interface with state (JavaBean properties) stored in the map.
 * <p/>
 * In current implementation calls to {@code hashCode()} and {@code toString()}
 * are delegated to the backing map. Two dynamic objects are equal if they are
 * of the same type (implement the same interface) and have the same state
 * (backing maps are equal).
 * <p/>
 * Primitive getters will throw an exception if the corresponding value is
 * {@code null}. One exception is primitive {@code boolean} getter, which in
 * this case will return {@code false}.
 *
...
public final class InterfaceMap {
  ...
  /**
   * Reflectively creates an instance of a given interface, that is backed by a
   * given map. The interface may contain JavaBean properties (getters and/or
   * setters) of the following types:
   * <ul>
   ...
   * <li>{@link java.util.List} of elements of any of the supported types (no
   * wildcards)</li>
   * <li>{@link java.util.Map} with {link String} keys and values of any of the
   * supported types (no wildcards)</li>
   * </ul>
   *
   * <p>
   * List getters return an immutable (possibly transformed) snapshot of a list
   * in the backing map.
   *
   ...
   *          type of the object to create
   *
   * @return instance of the {@code interfaceClass}, backed by
   *         {@code backingMap}
   *
