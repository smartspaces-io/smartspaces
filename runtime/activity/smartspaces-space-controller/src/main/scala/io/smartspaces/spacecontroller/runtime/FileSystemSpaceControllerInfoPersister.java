...
public class FileSystemSpaceControllerInfoPersister implements SpaceControllerInfoPersister {
  ...
  public void persist(SpaceController controllerInfo, SmartSpacesEnvironment spaceEnvironment) {
    ...
    props.put(
        io.smartspaces.spacecontroller.SpaceController.CONFIGURATION_NAME_CONTROLLER_DESCRIPTION,
        controllerInfo.getDescription());

    if (controllerInfo.getMetadata() != null) {
      props.put(
          io.smartspaces.spacecontroller.SpaceController.CONFIGURATION_NAME_CONTROLLER_METADATA,
          jsonMapper.toString(controllerInfo.getMetadata()));
    }

    File controllerInfoFile = new File(spaceEnvironment.getFilesystem().getInstallDirectory(),
        "config/smartspaces/controllerinfo.conf");
    ...
    try {
      ...
      writer.flush();
      spaceEnvironment.getLog().info(String.format("Persisted new controller information to %s",
