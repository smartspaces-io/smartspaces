...
public class JdomMasterDomainModelCreator implements MasterDomainDescription {
  ...
  public String newModel(ActivityRepository activityRepository,
    ...
    try {
      ...
      throw new SmartSpacesException("Could not create domain model", e);
    }
  }
  ...
  private Element newSpaceControllerEntry(SpaceController controller) {
    ...
    if (mode != null) {
      controllerElement.addContent(new Element(ELEMENT_NAME_SPACE_CONTROLLER_MODE).addContent(mode
          .name()));
    }
  ...
  /**
   * Add a space controller configuration to the given element if there is a
   * configuration.
   *
   * @param element
   ...
   */
  private void addSpaceControllerConfiguration(Element element,
    ...
    if (configuration != null) {
      ...
      if (!parameters.isEmpty()) {
        Element parametersElement =
            new Element(ELEMENT_NAME_SPACE_CONTROLLER_CONFIGURATION_ROOT_PARAMETERS);
  ...
  private Element newActivityEntry(Activity activity) {
    ...
    if (lastUploadDate != null) {
      activityElement.setAttribute(ATTRIBUTE_NAME_LAST_UPLOAD_DATE,
          Long.toString(lastUploadDate.getTime()));
    }

    Date lastStartDate = activity.getLastStartDate();
    if (lastStartDate != null) {
      activityElement.setAttribute(ATTRIBUTE_NAME_LAST_START_DATE,
          Long.toString(lastStartDate.getTime()));
    }
  ...
  private Element newLiveActivityEntry(LiveActivity liveActivity) {
    ...
    if (lastDeployDate != null) {
      liveActivityElement.addContent(new Element(ELEMENT_NAME_LIVE_ACTIVITY_LAST_DEPLOY_DATE)
          .addContent(Long.toString(lastDeployDate.getTime())));
    }
