...
import io.smartspaces.comm.CommunicationEndpoint;
...
public interface XBeeCommunicationEndpoint extends CommunicationEndpoint {
   ...
   *          transmission, if {@code 0} will be the maximum hops value as set
   *          by the {@code BH} parameter on the XBee
   * @param options
   *          options for the frame
   *
  ...
  /**
   * Construct an XBee Remote transmit XBee request with an unknown 16 bit
   * address.
   *
   * <p>
   * A frame number is automatically supplied by this endpoint.
  ...
  /**
   * Construct an XBree Remote Transmit Request if the 16 bit address isn't
   * known.
   *
   * @param address64
   *          the 64 bit destination address
   ...
   * @return the new request
   */
  TxRequestXBeeFrame newTxRequestXBeeFrame(XBeeAddress64 address64, int frameNumber,
      int broadcastRadius, int options);
