...
public class SpaceForm implements Serializable {
   ...
   */
  public void saveSpace(Space destinationSpace, ActivityRepository activityRepository) {
    destinationSpace.setName(space.getName());
    destinationSpace.setDescription(space.getDescription());

    destinationSpace.clearSpaces();
    if (spaceIds != null && !spaceIds.contains(UiUtilities.MULTIPLE_SELECT_NONE)) {
    ...
    if (liveActivityGroupIds != null
        && !liveActivityGroupIds.contains(UiUtilities.MULTIPLE_SELECT_NONE)) {
      for (String groupId : liveActivityGroupIds) {
        LiveActivityGroup group = activityRepository.getLiveActivityGroupById(groupId);
        if (group != null) {
