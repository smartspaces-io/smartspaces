...
/**
 * Invocation handler that stores state of the object (JavaBean properties) in
 * the backing map. Calls to hashCode() and toString() are delegated to the map.
 *
...
final class BackingMapInvocationHandler implements InvocationHandler {
  ...
  /**
   * The state of the object. Keys are JavaBean property names, values are
   * objects of the following:
   * <ul>
   * <li>a value of one of the {@link Conversions#SIMPLE_TYPE_CONVERSIONS}
   * <li>{@link Map} whose keys are Strings and values are found in this list of
   * types.
   ...
   * @param type
   *          type this BackingMapInvocationHandler represents (= the interface
   *          it implements)
   */
  ...
  public Object invoke(Object target, Method method, Object[] args) throws Throwable {
    ...
    if (method.getDeclaringClass() == Object.class) {
      ...
      // NOTE: clone() is not supported (yet?). Other methods (except
      // finalize()) are final.
    }
    ...
    throw new UnsupportedOperationException(String.format(
        "Method is neither a getter nor a setter: %s.", method));
  }
  ...
  /**
   * Perform a sanity check that a method is a setter, i.e. has a single
   * parameter.
   *
   * @param method
   *          setter method
   ...
   *           if the method has a setter-like name, but has number of
   *           parameters other than 1
   */
  private static boolean isSetter(Method method, Object[] params) {
    ...
    if (returnType != void.class && returnType != Void.class) {
      throw new UnsupportedOperationException(String.format("Setter method must return void: %s.",
          method));
    }

    if (params.length != 1) {
      throw new UnsupportedOperationException(String.format(
          "Setter method must have exactly 1 parameter: %s", method));
    }
  ...
  private static boolean isGetter(Method method, Object[] params) {
    ...
    if (params.length != 0) {
      throw new UnsupportedOperationException(String.format(
          "Getter method must be parameterless: %s", method));
    }
