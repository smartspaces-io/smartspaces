...
public class OsgiContainerResourceManager implements ContainerResourceManager, ManagedResource {
   ...
   * bootstrap folder.
   */
  private static final String SYSTEM_BOOTSTRAP_COMPONENT = "/"
   ...
   * bootstrap folder.
   */
  private static final String USER_BOOTSTRAP_COMPONENT = "/"
   ...
   */
  private final SmartSpacesFilesystem filesystem;
  ...
  public OsgiContainerResourceManager(BundleContext bundleContext, FrameworkWiring frameworkWiring,
      SmartSpacesFilesystem filesystem, File configFolder, ExtendedLog log) {
  ...
  @Override
  public void startup() {
    // Can't cache resources in here as cannot know if all bundles have been
    // loaded.
  }
  ...
  @Override
  public synchronized void addResource(ContainerResource incomingResource,
      ResourceSource resourceSource) {
    log.formatInfo("Adding container resource %s from URI %s", incomingResource,
    ...
    if (location.isImmediateLoad()) {
      ...
      if (installedBundle != null) {
        if (installedBundle.getLocation().equals(resourceDestinationFileUri)) {
          updateExactInstalledBundle(incomingResource, resourceSource, resourceDestinationFile,
              installedBundle);
        } else {
          swapInstalledBundle(incomingResource, resourceSource, resourceDestinationFile,
              resourceDestinationFileUri, installedBundle);
        }

      } else {
        loadNewBundle(incomingResource, resourceSource, resourceDestinationFile,
            resourceDestinationFileUri);
      }
    } else {
  ...
  public synchronized Bundle loadAndStartBundle(File bundleFile, ContainerResourceType type)
    ...
    if (!fileSupport.isFile(bundleFile)) {
      throw SimpleSmartSpacesException
          .newFormattedException("Could not find bundle file %s of type %s",
    ...
    try {
      ...
      // if managed to install the bundle then it should be uninstalled since we
      // are in an error.
      if (bundle != null) {
        try {
          bundle.uninstall();
          ...
          log.formatError(e,
              "Could not uninstall an OSGi bundle that could not be started: %s of type",
              fileSupport.getAbsolutePath(bundleFile), type);
        }
      }

      throw SmartSpacesException.newFormattedException(e, "Cannot load bundle file %s of type %s",
          type, fileSupport.getAbsoluteFile(bundleFile), type);
    }
  }
  ...
  public synchronized void uninstallBundle(Bundle bundle) {
    ...
    try {
      ...
      throw SmartSpacesException.newFormattedException(e, "Cannot unload bundle %s", bundleUri, e);
    }
  }
   ...
   */
  private void loadNewBundle(ContainerResource resource, ResourceSource resourceSource,
      File resourceDestinationFile, String resourceDestinationFileUri) {
    ...
    try {
      ...
      throw SmartSpacesException.newFormattedException(e,
          "Could not load a new bundle %s from source %s", resourceDestinationFileUri,
          resourceSource.getLocation());
    }
  }

  /**
   * Update the bundle that has the exact same filename as the resource coming
   * in.
   *
   ...
   */
  private void updateExactInstalledBundle(ContainerResource incomingResource,
      ResourceSource resourceSource, File resourceDestinationFile, Bundle installedBundle) {
    log.formatDebug("Resource %s already was loaded. Attempting update.", incomingResource);

    ContainerResource existingResource =
        getContainerResource(getBundleUri(installedBundle).toString());
    ...
    Collection<Bundle> activitiesDependentOnBundle =
        getLoadedActivitiesDependentOnBundle(installedBundle);
  ...
  /**
   * Get a list of all activity bundles that are dependent on the dependency
   * bundle.
   *
  ...
  private URI getBundleUri(Bundle bundle) {
    try {
      ...
      throw SimpleSmartSpacesException.newFormattedException(e, "Could not parse URI for %s",
          bundle.getLocation());
    }
  }
  ...
  private void swapInstalledBundle(ContainerResource resource, ResourceSource resourceSource,
    ...
    log.formatDebug(
        "Resource %s already was loaded with another name. Uninstalling old and loading new.",
        resource);

    // No matter what, copy the new bundle without checking the signature since
    // we want it with its new name.
    resourceSource.copyTo(resourceDestinationFile);
    ...
    try {
      ...
      throw SmartSpacesException.newFormattedException(e, "Could not swap a resource %s", resource);
    }
  }
   ...
   */
  private File getResourceDestinationFolder(ContainerResourceLocation location)
      throws SmartSpacesException {
    switch (location) {
      ...
      case CONFIG:
        if (configFolder == null) {
          throw new SimpleSmartSpacesException("Configurations are not modifiable");
        }

        return fileSupport.newFile(configFolder,
            ContainerFilesystemLayout.FOLDER_CONFIG_smartspaces);
      ...
      default:
        throw SimpleSmartSpacesException.newFormattedException("Unsupported container location %s",
            location);
    }
  }
   ...
   *          the resource
   *
   * @return the bundle for the resource, or {@code null} if no bundles are
   *         available that provide the resource
   */
  private Bundle locateBundleForResource(ContainerResource resource) {
    ...
    for (Bundle bundle : bundleContext.getBundles()) {
      if (bundle.getSymbolicName().equals(resource.getName())
          && bundle.getVersion().equals(osgiVersion)) {
        return bundle;
      }
    }
   ...
   *          the bundle URI
   *
   * @return the container resource, or {@code null} if no such bundle being
   *         tracked
   */
  @VisibleForTesting
   ...
   * @return the updater
   */
  private BundleUpdater newBundleUpdater(Bundle bundle, ContainerResource existingResource,
      String signatureNew) {
   ...
   */
  private synchronized void completeBundleUpdate(Bundle bundle, ContainerResource existingResource,
      String newSignature) {
  ...
  public class BundleUpdater implements FrameworkListener {
    ...
    public void updateBundle() {
      ...
      try {
        ...
        try {
          if (!doneUpdateLatch.await(BUNDLE_UPDATER_TIMEOUT, TimeUnit.MILLISECONDS)) {
            throw new SimpleSmartSpacesException("Could not update bundle in time");
          }
      ...
      if (throwable != null) {
        throw new SmartSpacesException("Could not update resource", throwable);
      } else {
