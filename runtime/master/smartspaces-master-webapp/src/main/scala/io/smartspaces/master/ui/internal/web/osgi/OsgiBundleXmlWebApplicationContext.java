...
package io.smartspaces.master.ui.internal.web.osgi;
 ...
 * mandates that the OSGi bundle context is either set (
 * {@link #setBundleContext(BundleContext)} before setting the ServletContext or
 * that the given ServletContext contains the BundleContext as an attribute
 * under {@link #BUNDLE_CONTEXT_ATTRIBUTE} (
 * <code>org.springframework.osgi.web.org.osgi.framework.BundleContext</code>).
 *
 * <p/>
 * Additionally, this implementation replaces the {@link ServletContext}
...
public class OsgiBundleXmlWebApplicationContext extends OsgiBundleXmlApplicationContext implements
   ...
   */
  private static final String APPLICATION_CONTEXT_SERVICE_NAMESPACE_PROPERTY =
  ...
  public OsgiBundleXmlWebApplicationContext() {
    if (WebappActivator.getBundleContext() != null) {
      setBundleContext(WebappActivator.getBundleContext());
    }
  ...
  protected void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory) {
    ...
    // System.out.println("Hey, I gotta " + bundleContext.getService(sr));
    // }
    // .getServiceReference(ConfigurableOsgiBundleApplicationContext.class.getName());
    // ConfigurableOsgiBundleApplicationContext foo =
    // (ConfigurableOsgiBundleApplicationContext)getBundleContext().getService(sr);
    // System.out.println("Got foo " + foo);
    // System.out.println("BasicAlertServiceTest got a " +
    // foo.getBundleContext());
   ...
   * {@inheritDoc}
   *
   * Sets the config locations for this application context in init-param style,
