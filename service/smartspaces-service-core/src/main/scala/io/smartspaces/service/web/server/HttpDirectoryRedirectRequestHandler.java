 ...
 */
public class HttpDirectoryRedirectRequestHandler implements HttpGetRequestHandler {
   ...
   */
  public HttpDirectoryRedirectRequestHandler(String requestPrefix, File contentDirectory,
  ...
  public void handleGetHttpRequest(HttpRequest request, HttpResponse response) {
    try {
      ...
      if (!fullPath.startsWith(requestPrefix)) {
        throw new SimpleSmartSpacesException("Bad path prefix: " + fullPath);
      }
      ...
      if (absFile.isDirectory()) {
        ...
        String effectiveTarget =
            needAdjustedTarget ? (fullPath + "/" + redirectTarget) : redirectTarget;
