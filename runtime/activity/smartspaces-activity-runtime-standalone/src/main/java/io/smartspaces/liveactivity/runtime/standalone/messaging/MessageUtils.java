...
public final class MessageUtils {
  ...
  /**
   * Verify if a message matches a template. The template is a data object
   * (consisting of lists, items, or maps), that should verify/match the input
   * target. At each level, the entries should be identical -- except in the
   * case of a list, for which the target merely needs to contain all the items
   * in the template (but may have more). Extra fields in the target with no
   * corresponding template entry are OK and will not prevent a match. Any type
   * mismatch will result in a falure to match (unless .equals() works for the
   * object).
   *
   * The current implementation does not correctly work with lists of complex
   * items (such as lists of maps), but there is no real use case for that and
   * so it's not critical. Typically a list will be a list of simple objects
   * (e.g, strings).
   *
   * @param target
   *          message to match
  ...
  /**
   * Make message suitable for tracing, given the input message and message
   * whitelist. This process involves first checking to see if it's a matched
   * trace message, and if so, then redacting any necessary parts.
   *
   * @param message
  ...
  static MessageSetList readMessageList(File inputFile) {
    ...
    try {
      ...
      while ((line = checkReader.readLine()) != null) {
        ...
        if (parts.length > 2) {
          throw new SimpleSmartSpacesException("Excess parts to message line: " + line);
        }
        messageList.add(messageSet);
      }
      checkReader.close();
    } catch (Exception e) {
      throw new SimpleSmartSpacesException("While processing check file "
