...
package io.smartspaces.workbench.project.activity.packager;
...
public class StandardActivityProjectPackager implements ActivityProjectPackager {
  ...
  private void buildZip(Project project, ProjectTaskContext context) {
    ...
    try {
      ...
      throw new SmartSpacesException("Failed writing Activity Build file", e);
    } finally {
      if (out != null) {
        try {
          out.close();
   ...
   */
  private void addArtifacts(ProjectTaskContext context, byte[] buf, ZipOutputStream out)
      throws IOException {
  ...
  private File getBuildDestinationFile(Project project, File buildFolder) {
    return fileSupport.newFile(buildFolder,
        project.getIdentifyingName() + "-" + project.getVersion() + "."
