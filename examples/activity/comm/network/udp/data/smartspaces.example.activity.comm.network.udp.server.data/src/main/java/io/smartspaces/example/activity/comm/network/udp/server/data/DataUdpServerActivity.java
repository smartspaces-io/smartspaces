...
package io.smartspaces.example.activity.comm.network.udp.server.data;
...
/**
 * A Smart Spaces Java-based activity that demonstrates using a UDP server with
 * binary data.
 *
 * <p>
 * A simple data message of an integer followed by a float is received and a
 * response is generated consisting of the float multiplied by 3 followed by the
 * int multiplied by 5.
 *
...
public class DataUdpServerActivity extends BaseActivity {
  ...
  public void onActivitySetup() {
    ...
    UdpServerNetworkCommunicationEndpoint udpServer =
        communicationEndpointService.newServer(serverPort, getLog());
  ...
  private void handleUdpRequest(UdpServerRequest request) {
    // The items must be read in the order written by the sender, or the reads
    // need to specify the exact locations
    // of the data.
    ...
    getLog().info(
        String.format("UDP server got request from client %s: int is %d, float is %f",
            request.getRemoteAddress(), intNumber, floatNumber));
