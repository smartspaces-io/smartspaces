 ...
 *
 * <p>
 * One way to test is to have the ROS command line tools installed and to use
 * the command
 *
 * <pre>
...
public class CommRosHelloSubscriberActivity extends BaseActivity {
  ...
  public void onActivityStartup() {
    // No need to shutdown the subscriber, this will happen as part of node
    // shutdown which shuts down all
  ...
  private void onNewRosMessage(ColorRGBA message) {
    getLog().info(
        String.format("Received ROS message {r: %f, g: %f, b: %f, a: %f} on topic %s",
            message.getR(), message.getG(), message.getB(), message.getA(), topicName));
  }
}
