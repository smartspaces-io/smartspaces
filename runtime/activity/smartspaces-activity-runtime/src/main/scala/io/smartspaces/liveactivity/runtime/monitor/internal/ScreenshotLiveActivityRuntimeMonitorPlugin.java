 ...
 */
public class ScreenshotLiveActivityRuntimeMonitorPlugin extends
    BaseLiveActivityRuntimeMonitorPlugin {

  /**
   * The name of the directory inside the container's tmp folder where
   * screenshots will live.
   */
  public static final String SCREENSHOT_DIRECTORY_NAME = "screenshots";

  /**
   * The prefix of the default executable name for screenshots. It will have the
   * operating system tacked onto the end.
   */
  public static final String SCREENSHOT_EXECUTABLE_PREFIX_DEFAULT = "screenshot.";

  /**
   * The sub folder in the controller library folders where the default
   * screenshot scripts are kept.
   */
   ...
   */
  private static final byte[] RESPONSE_MESSAGE_SCREENSHOT_SUCCESS =
      "<p>The screenshot succeeded.</p>".getBytes();

  /**
   * HTML message for a failed screenshot.
   */
  private static final byte[] RESPONSE_MESSAGE_SCREENSHOT_FAILURE =
      "<p style=\"color:red\">The screenshot failed! See controller logs for details.</p>"
          .getBytes();
  ...
  public static final Comparator<File> FILE_COMPARATOR_DATE_DESCENDING = new Comparator<File>() {
    ...
    public int compare(File o1, File o2) {
      // Math because modified times are longs and we need an int. Otherwise
      // would just subtract and return.
  ...
  private List<PluginFunctionalityDescriptor> functionalityDescriptors = Collections
      .unmodifiableList(Lists.newArrayList(new PluginFunctionalityDescriptor(URL_PREFIX_SCREENSHOT,
          "Machine Screenshot")));
  ...
  public void onStartup() {
    ...
    SmartSpacesEnvironment spaceEnvironment = liveActivityRuntime.getSpaceEnvironment();
    ...
    screenshotFolder =
        liveActivityRuntime.getSpaceEnvironment().getFilesystem()
            .getTempDirectory(SCREENSHOT_DIRECTORY_NAME);
  ...
  @Override
  protected void onHandleRequest(HttpRequest request, HttpResponse response, String fullPath)
      throws Exception {
    ...
    if (requestPathComponents.length == 1) {
      ...
      // If here, the URL was something like /base/filename where base was the
      // value of the constant
      // URL_PREFIX_SCREENSHOT. The request path above then becomes /filename.
      // The split will have an empty string in
      // the first component of the array and the filename is the second
      // component, hence the 1 below.
      showScreenshot(response, requestPathComponents[1]);
    }
  }

  /**
   * Do the screenshot and then show an index of the directory containing all
   * screenshots.
   *
   * @param response
  ...
  private void reportLackOfScreenshotExecutable(HttpResponse response) throws IOException {
    getMonitorService().getLog().warn(
        "Screenshot executable is null in live activity runtime remote monitor");
   ...
   */
  private void showDirectoryIndex(HttpResponse response, boolean captureSuccessful)
      throws IOException {
    OutputStream outputStream = response.getOutputStream();
    ...
    listDirectoryFiles(URL_PREFIX_SCREENSHOT, screenshotFolder, outputStream,
        FILE_COMPARATOR_DATE_DESCENDING);
  ...
  private void showScreenshot(HttpResponse response, String screenshotFilename) throws IOException {
    ...
    if (fileSupport.exists(screenshotFile)) {
      ...
      String message =
          String.format("Screenshot file not found: %s",
              fileSupport.getAbsolutePath(screenshotFile));
  ...
  private boolean captureScreenshots() throws InterruptedException {
    ...
    Date now =
        new Date(liveActivityRuntime.getSpaceEnvironment().getTimeProvider().getCurrentTime());
    String screenshotFilenamePrefix =
        SCREENSHOT_FILE_PREFIX + SCREENSHOT_FILENAME_DATETIME_FORMATTER.format(now);

    description.parseArguments(escapedSceenshotFolderFilepath).addArguments(
        screenshotFilenamePrefix, SCREENSHOT_FILE_EXTENSION);

    NativeApplicationRunnerCollection runnerCollection =
        new StandardNativeApplicationRunnerCollection(liveActivityRuntime.getSpaceEnvironment(),
            liveActivityRuntime.getSpaceEnvironment().getLog());

    try {
      // TODO(keith): Once used, should this runner collection stay running
