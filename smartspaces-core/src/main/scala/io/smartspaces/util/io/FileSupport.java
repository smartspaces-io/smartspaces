...
public interface FileSupport {
   ...
   */
  void addFileToZipStream(ZipOutputStream zipOutputStream, File basePath, File relPath,
      String pathPrefix);
   ...
   *          the source directory
   * @param filter
   *          the file filter that determines which files get copied, can be
   *          {@code null} which passes everything
   * @param destDir
   *          the destination directory (which will be created if necessary)
   * @param overwrite
   *          {@code true} if should overwrite files if already in the
   ...
   *          if not {@code null}, used to collect copied files
   */
  void copyDirectory(File sourceDir, FileFilter filter, File destDir, boolean overwrite,
   ...
   *          the output stream
   * @param closeOnCompletion
   *          {@code true} if the streams should be closed on completion of the
   *          copy
   *
   * @throws IOException
   *           io problem with operation
   ...
   */
  void directoryExists(File dir) throws SimpleSmartSpacesException;
   ...
   *           if the result is not the existence of a valid directory
   */
  void directoryExists(File dir, String message) throws SimpleSmartSpacesException;
   ...
   *
   * <p>
   * This method takes canonical pathnames and corrects them for the underlying
   * operation system.
   *
   * @param path
   *          the path
  ...
  /**
   * Create a new file for a specified path relative to the parent file. if the
   * subpath is absolute, the returned file will be just the subpath.
   *
   * <p>
   * This method takes canonical pathnames and corrects them for the underlying
   * operation system.
  ...
  /**
   * Create a unique temporary file in the given directory, using default prefix
   * and suffix.
   *
   * @param baseDir
  ...
  /**
   * Create a unique temporary directory in the given directory, using default
   * prefix and suffix.
   *
   * @param baseDir
  ...
  /**
   * Close the closeable. If asked to throw any exceptions from the close, the
   * method will wrap it in an {@link SmartSpacesException}.
   *
   * @param closeable
   *          the item to close, can be {@code null}
   * @param throwException
   *          {@code true} if any exceptions thrown during closing should be
   *          rethrown
   *
   * @throws SmartSpacesException
   *           an exception happened during close
   */
  void close(Closeable closeable, boolean throwException) throws SmartSpacesException;

  /**
   * Is the candidate parent the parent of the file?
   *
   * <p>
   * The parent relationship is a strict parent. {@code /foo/bar} is not
   * considered a parent for {@code /foo/bart/spam} and {@code /foo/bar} is not
   * a parent for {@code /foo/bar}.
   *
