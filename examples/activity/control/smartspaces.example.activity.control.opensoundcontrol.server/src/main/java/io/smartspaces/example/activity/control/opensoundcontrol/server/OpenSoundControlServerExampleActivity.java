...
/**
 * A Smart Spaces Java-based activity that plays audio tracks based on Open
 * Sound Control messages coming into an OSC server.
 *
 * <p>
...
public class OpenSoundControlServerExampleActivity extends BaseActivity {
  ...
  /**
   * The name of the config property for the Open Sound Control address that
   * will invoke the play method.
   */
  public static final String CONFIGURATION_PROPERTY_OPEN_SOUND_CONTROL_METHOD_ADDRESS =
      "space.opensoundcontrol.methodaddress";

  /**
   * The name of the config property for the Open Sound Control track map, which
   * maps sound IDs to tracks to be played.
   */
  public static final String CONFIGURATION_PROPERTY_OPEN_SOUND_CONTROL_TRACK_MAP =
      "space.opensoundcontrol.trackmap";
  ...
  public void onActivitySetup() {
    ...
    int serverPort =
        getConfiguration().getRequiredPropertyInteger(
            CONFIGURATION_PROPERTY_OPEN_SOUND_CONTROL_SERVER_PORT);

    OpenSoundControlServerCommunicationEndpoint oscServer =
        serverService.newUdpEndpoint(serverPort, getLog());
    addManagedResource(oscServer);

    String methodAddress =
        getConfiguration().getRequiredPropertyString(
            CONFIGURATION_PROPERTY_OPEN_SOUND_CONTROL_METHOD_ADDRESS);
  ...
  private void populateTrackMap() {
    List<String> trackMap =
        getConfiguration().getPropertyStringList(
            CONFIGURATION_PROPERTY_OPEN_SOUND_CONTROL_TRACK_MAP, TRACKMAP_SEPARATOR);
