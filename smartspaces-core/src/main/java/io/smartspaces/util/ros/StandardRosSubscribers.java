...
public class StandardRosSubscribers<T> implements RosSubscribers<T> {
  ...
  @Override
  public synchronized void addSubscribers(ConnectedNode node, String messageType,
      Set<String> topicNames, MessageListener<T> listener) {
  ...
  public void onMasterRegistrationFailure(Subscriber<T> subscriber) {
    log.warn(String.format("Subscriber for topic %s has failed to register with the master",
  ...
  public void onMasterUnregistrationSuccess(Subscriber<T> subscriber) {
    log.debug(String.format("Subscriber for topic %s has unregistered from the master",
        subscriber.getTopicName()));
  }

  @Override
  public void onNewPublisher(Subscriber<T> subscriber, PublisherIdentifier publisher) {
    log.debug(String.format("Subscriber for topic %s has a new publisher",
        subscriber.getTopicName()));
  }
