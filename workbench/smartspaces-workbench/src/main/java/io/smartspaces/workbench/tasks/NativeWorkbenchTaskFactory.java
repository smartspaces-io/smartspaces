...
public class NativeWorkbenchTaskFactory implements WorkbenchTaskFactory {
  ...
  public WorkbenchTask newTask() {
    return new BaseWorkbenchTask() {
      @Override
      public void perform(WorkbenchTaskContext workbenchTaskContext) throws SmartSpacesException {
        ...
        try {
          ...
          SimpleSmartSpacesException.throwFormattedException("Native command %s interrupted",
              applicationDescription);
        }

        NativeApplicationRunnerState state = runner.getState();
        if (!NativeApplicationRunnerState.SHUTDOWN.equals(state)) {
          SimpleSmartSpacesException.throwFormattedException("Native command %s ended in state %s",
   ...
   *
   * <p>
   * This method is meant to be overridden. By default it returns the workbench
   * configuration.
   *
  ...
  protected File getBaseTemplateDirectory(WorkbenchTaskContext workbenchTaskContext) {
    return workbenchTaskContext.getWorkbench().getSpaceEnvironment().getFilesystem()
        .getTempDirectory();
  }
  ...
  private File createTemplateFile(File baseDirectory) {
    return fileSupport.createTempFile(baseDirectory, TEMPLATE_FILE_NAME_PREFIX,
      ...
      if (templateContent == null || templateContent.isEmpty()) {
        ...
        if (fileSupport.isFile(templateSourceFile)) {
          ...
          SimpleSmartSpacesException.throwFormattedException(
              "Could not find task template file %s", templateSourceFile.getAbsolutePath());
        }
      }

      String evaluatedContent =
          evaluateTemplateContent(workbenchTaskContext, config, templateContent);
  ...
  private String evaluateTemplateContent(WorkbenchTaskContext workbenchTaskContext,
    ...
    String templatedContent =
        workbenchTaskContext.getWorkbench().getTemplater().processStringTemplate(data, content);
