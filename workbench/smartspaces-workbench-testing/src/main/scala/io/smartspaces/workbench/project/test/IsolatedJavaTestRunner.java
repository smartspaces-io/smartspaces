...
public class IsolatedJavaTestRunner {
  ...
  public boolean runTests(File testCompilationFolder, URLClassLoader classLoader, Log log) {
    ...
    if (!testClasses.isEmpty()) {
      ...
      log.warn(String.format("No JUnit test classes found in %s",
          testCompilationFolder.getAbsolutePath()));
  ...
  private boolean runJunitTests(List<Class<?>> testClasses, Log log) {
    ...
    for (Class<?> testClass : testClasses) {
      try {
        ...
        log.info(String.format("Ran %2d tests in %4dms: %d failed, %d ignored. (%s)",
            result.getRunCount(), result.getRunTime(), result.getFailureCount(),
            result.getIgnoreCount(), testClass.getName()));
