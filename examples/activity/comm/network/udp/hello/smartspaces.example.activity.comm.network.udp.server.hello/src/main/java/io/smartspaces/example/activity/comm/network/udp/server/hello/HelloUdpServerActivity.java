...
package io.smartspaces.example.activity.comm.network.udp.server.hello;
...
/**
 * A Smart Spaces Java-based activity that demonstrates using a UDP server.
 *
 * <p>
...
public class HelloUdpServerActivity extends BaseActivity {
  ...
  private void handleUdpRequest(UdpServerRequest request) {
    ...
    getLog().info(
        String.format("UDP server got request from client %s: %s", request.getRemoteAddress(),
            message));
