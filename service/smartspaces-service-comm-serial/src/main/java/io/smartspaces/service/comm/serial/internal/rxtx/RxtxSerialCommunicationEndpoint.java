...
public class RxtxSerialCommunicationEndpoint implements SerialCommunicationEndpoint {
  ...
  static {
    ...
    SERIAL_PORT_FLOW_CONTROL_VALUES
        .put(FlowControl.XONXOFF_OUT, SerialPort.FLOWCONTROL_XONXOFF_OUT);
  }
  /**
  ...
  public void startup() {
    try {
      ...
      throw new SmartSpacesException(String.format("Unable to connect to serial port %s with rxtx",
          portName), e);
    }
  }
  ...
  public String toString() {
    return "RxtxSerialCommunicationEndpoint [portName=" + portName + ", baud=" + baud
   ...
   */
  private SerialPort createSerialPort(String portName) throws SmartSpacesException {
    ...
    while (portIdentifiers.hasMoreElements()) {
      ...
      if (candidatePortIdentifier.getPortType() == CommPortIdentifier.PORT_SERIAL
          && candidatePortIdentifier.getName().equals(portName)) {
        portIdentifier = candidatePortIdentifier;
        break;
      }
    }
    if (portIdentifier == null) {
      throw new SimpleSmartSpacesException("Could not find serial port " + portName);
    }
    ...
    try {
      ...
      port = (SerialPort) portIdentifier.open("smartspaces", TIME_TO_WAIT_FOR_PORT);
    } catch (PortInUseException e) {
      throw new SimpleSmartSpacesException("Serial port already in use: " + portName);
    }
