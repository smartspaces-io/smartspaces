...
public class StandardLiveActivityRuntimeTest {
  ...
  public void setup() {
    ...
    when(liveActivityRuntimeComponentFactory.newLiveActivityRunnerFactory()).thenReturn(
  ...
  private void tryActivityStateAction(ActivityAction action, ActivityState startState,
    ...
    when(liveActivityRepository.getInstalledLiveActivityByUuid(activityUuid)).thenReturn(
        liveActivity);

    InternalLiveActivityFilesystem activityFilesystem = mock(InternalLiveActivityFilesystem.class);
    when(liveActivityStorageManager.getActivityFilesystem(activityUuid)).thenReturn(
        activityFilesystem);
    LiveActivityConfiguration configuration = mock(LiveActivityConfiguration.class);

    when(
        liveActivityConfigurationManager.newLiveActivityConfiguration(liveActivity,
            activityFilesystem)).thenReturn(configuration);

    StandardLiveActivityRunner expectedActive = mock(StandardLiveActivityRunner.class);
    when(
        liveActivityRunnerFactory.newLiveActivityRunner(liveActivity, activityFilesystem,
            configuration, liveActivityRuntime, liveActivityRuntime)).thenReturn(expectedActive);
    ...
    verify(liveActivityStatusPublisher, times(1)).publishActivityStatus(eq(activityUuid),
  ...
  public void testCleanLiveActivityTempDataDir() {
    ...
    StandardLiveActivityRunner active =
        new StandardLiveActivityRunner(installedActivity, null, null, null, liveActivityRuntime,
            liveActivityRuntime);
  ...
  /**
   * Test an activity successfully starting up through the activity event
   * listening channels.
   */
  @Test
  public void testActivityStartupSuccess() {
    ...
    liveActivityRuntime.getActivityListener()
        .onActivityStatusChange(activity, oldStatus, newStatus);
  ...
  /**
   * Test an activity failing to start up through the activity event listening
   * channels.
   */
  @Test
  ...
  /**
   * Test an activity successfully activating through the activity event
   * listening channels.
   */
  @Test
  ...
  /**
   * Test an activity failing to activate through the activity event listening
   * channels.
   */
  @Test
