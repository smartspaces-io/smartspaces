...
package io.smartspaces.example.activity.comm.network.udp.client.hello;
...
/**
 * A Smart Spaces Java-based activity which demonstrates using a UDP client.
 *
 * <p>
...
public class HelloUdpClientActivity extends BaseActivity {
  ...
  private void handleUdpResponse(byte[] response, InetSocketAddress remoteAddress) {
    ...
    getLog().info(
        String.format("UDP client got response from server %s: %s", remoteAddress, message));
  }
}
