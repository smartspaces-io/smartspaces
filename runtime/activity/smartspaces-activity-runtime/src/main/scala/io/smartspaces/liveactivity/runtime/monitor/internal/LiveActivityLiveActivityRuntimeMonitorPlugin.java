 ...
 */
public class LiveActivityLiveActivityRuntimeMonitorPlugin extends
    BaseLiveActivityRuntimeMonitorPlugin {

  /**
   * String formatting string for directory listing for a live activity's file
   * system.
   */
  ...
  /**
   * The location in the request path components for the file path into the
   * section.
   */
  ...
  private List<PluginFunctionalityDescriptor> functionalityDescriptors = Collections
      .unmodifiableList(Lists.newArrayList(new PluginFunctionalityDescriptor(URL_PREFIX_ACTIVITY,
          "Activities")));
   ...
   *           an exception happened while processing
   */
  private void writeActivityInformationPage(HttpRequest request, HttpResponse response)
      throws Throwable {

    String requestPath =
        request.getUri().getPath().substring(URL_PREFIX_ACTIVITY_FILESYSTEM.length());
   ...
   */
  private void displaySpecificDirectorySection(HttpResponse response,
      String[] requestPathComponents, String uuid) throws Throwable {
    ...
    switch (section) {
      ...
      default:
        SimpleSmartSpacesException.throwFormattedException(
            "Unknown filesystem area %s for live activity %s", section, uuid);
    }
    displayFilesystemSection(response, baseUrl, section, requestPathComponents,
   ...
   *           there is no live activity with the given UUID or something failed
   *           when getting the file system
   */
  private InternalLiveActivityFilesystem getLiveActivityFilesystem(String uuid)
      throws SmartSpacesException {
    ...
    if (liveActivityStorageManager.getAllInstalledActivityUuids().contains(uuid)) {
      ...
      throw SimpleSmartSpacesException.newFormattedException(
          "There is no installed live activity with UUID %s", uuid);
    }
  }
  ...
  private void displayAllSectionsListing(HttpResponse response, String uuid) throws Throwable {
    ...
    addCommonPageHeader(outputStream,
        String.format(STRING_FORMAT_DIRECTORY_LISTING_FOR_LIVE_ACTIVITIES, uuid));
