 ...
 *
 * <p>
 * This implementation supports the concept of multiple evaluation passes. This
 * is useful when cascading definitions need to be resolved in the output. Say,
 * for a project definition, there is a concept of {@code packageName=$
 * directoryName}.${className}} and then the template itself references
 * {@code $ packageName} , then the first pass will resolve packageName, and the
 * second pass will resolve ${directoryName} and ${className}. Since this is a
 * templating language, evaluations are not recursive, and so this is necessary
 * to properly handle the output.
 *
...
public class FreemarkerTemplater implements ManagedResource {
   ...
   */
  private SmartSpacesWorkbench workbench;
   ...
   *          the workbench
   */
  public FreemarkerTemplater(SmartSpacesWorkbench workbench) {
  ...
  public synchronized void startup() {
    try {
      ...
      throw new SmartSpacesException("Cannot initialize activity project creator", e);
    }
  }
   ...
   * @param defineResult
   *          target value to define with new value, or {@code null} if no
  ...
  public String processStringTemplate(Map<String, Object> data, String templateContent) {
    try {
      Template temp =
          new Template("generator for " + templateContent, new StringReader(templateContent),
              getConfiguration());
      StringWriter stringWriter = new StringWriter();
      temp.process(data, stringWriter);
      return stringWriter.toString();
    } catch (Exception e) {
      throw new SmartSpacesException(String.format("Could not instantiate string template %s",
          templateContent), e);
    }
  }
  ...
  public void writeTemplate(Map<String, Object> data, File outputFile, String template,
    ...
    for (int passesRemaining = evaluationPasses; passesRemaining > 0; passesRemaining--) {
      tempFile =
          fileSupport.newFile(String.format("%s.%d", outputFile.getAbsolutePath(),
              passesRemaining - 1));
      deleteList.add(tempFile);
    ...
    // By design. if there are any errors processing the templates, these files
    // will remain.
  ...
  public void writeTemplate(Map<String, Object> data, File outputFile, String template) {
    ...
    try {
      ...
      throw new SimpleSmartSpacesException(String.format("Could not instantiate template %s to %s",
