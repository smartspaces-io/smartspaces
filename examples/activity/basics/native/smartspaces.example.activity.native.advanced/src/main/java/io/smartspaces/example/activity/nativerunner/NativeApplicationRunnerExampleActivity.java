...
package io.smartspaces.example.activity.nativerunner;
...
import io.smartspaces.util.process.BaseNativeApplicationRunnerListener;
...
/**
 * An example activity which shows how to use the
 * {@link NativeApplicationRunner}.
 *
...
public class NativeApplicationRunnerExampleActivity extends BaseActivity {
   ...
   */
  public static final String CONFIGURATION_APPLICATION_EXECUTABLE =
      "space.nativeapplication.executable";

  /**
   * Configuration property giving the flags that a native activity would use to
   * launch.
   */
  public static final String CONFIGURATION_APPLICATION_EXECUTABLE_FLAGS =
      "space.nativeapplication.executable.flags";
  ...
  public void onActivitySetup() {
    nativeAppRunnerCollection =
        new StandardNativeApplicationRunnerCollection(getSpaceEnvironment(), getLog());
  ...
  public void onActivityActivate() {
    ...
    // The Workbench build system does not zip files with their executable bit
    // set, so if the executable is
    // part of the activity build then we need to make sure that the executable
    // bit gets set before we
    // try running it. We will only set the bit if it is in the activity's
    // install folder, otherwise this is a bit
    // of a security hole.
    ActivityFilesystem activityFilesystem = getActivityFilesystem();
    File executableFile =
        fileSupport.resolveFile(activityFilesystem.getInstallDirectory(), executablePath);
    if (fileSupport.isParent(activityFilesystem.getInstallDirectory(), executableFile)) {
      if (fileSupport.exists(executableFile)) {
        ...
        throw new SimpleSmartSpacesException(
            String.format("The native application executable %s does not exist",
    ...
    // If extending BaseNativeApplicationRunnerListener, it is not necessary to
    // override every
    ...
    runner.addNativeApplicationRunnerListener(new BaseNativeApplicationRunnerListener() {
      ...
      public boolean onNativeApplicationRunnerShutdownRequested(NativeApplicationRunner runner) {
        ...
        // This method is not handling the shutdown. If true is returned, this
        // method could tell the
      ...
      public boolean onRestartAttempt(RestartStrategy<NativeApplicationRunner> restartStrategy,
        ...
        // true means that the strategy should attempt a restart. If we should
        // punt on restarting, then return false.
        return true;
      }
