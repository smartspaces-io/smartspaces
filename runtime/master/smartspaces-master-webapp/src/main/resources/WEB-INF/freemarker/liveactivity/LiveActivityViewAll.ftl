...
function shutdownAllActivitiesAllControllers() {
    if (confirm("Are you sure you want to shut down all live activities on all space controllers?")) {
        window.location='/smartspaces/spacecontroller/all/activities/shutdown.html';
    }
}
...
<table class="commandBar">
  <tr>
    ...
    <td><button type="button" id="newButton" onclick="window.location='/smartspaces/liveactivity/new.html?mode=embedded'" title="${title}" ${disabledAttribute}>New</button></td>
    <td><button type="button" id="nstatusAllButton" onclick="doAjaxCommandByUrl('/smartspaces/spacecontroller/all/status.json')" title="Get the status of all Live Activities on all Space Controllers">Status All</button></td>
...
<#list liveactivities as liveactivity>
  ...
  <tr class="${trCss}">
    <td class="liveactivity-name">
      <a class="uglylink" onclick="return ugly.changePage('/smartspaces/liveactivity/${liveactivity.id}/view.html', event);">${liveactivity.name}</a>
    </td>
    <td>
