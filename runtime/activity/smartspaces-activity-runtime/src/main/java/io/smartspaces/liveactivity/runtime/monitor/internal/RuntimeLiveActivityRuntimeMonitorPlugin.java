...
public class RuntimeLiveActivityRuntimeMonitorPlugin extends BaseLiveActivityRuntimeMonitorPlugin {
  ...
  /**
   * The position in the request path components for the beginning of the file
   * path relative to the root directory of the section.
   */
  ...
  private List<PluginFunctionalityDescriptor> functionalityDescriptors = Collections
      .unmodifiableList(Lists.newArrayList(new PluginFunctionalityDescriptor(
          URL_PREFIX_RUNTIME_FILESYSTEM, "Runtime Filesystem")));
   ...
   */
  private void writeRuntimeInformationPage(HttpRequest request, HttpResponse response)
      throws Throwable {

    String requestPath =
        request.getUri().getPath().substring(URL_PREFIX_RUNTIME_FILESYSTEM.length());
   ...
   */
  private void
      displaySpecificDirectorySection(HttpResponse response, String[] requestPathComponents)
          throws Throwable {
    SmartSpacesFilesystem filesystem =
    ...
    displayFilesystemSection(response, URL_PREFIX_RUNTIME_FILESYSTEM, section,
        requestPathComponents, PATH_COMPONENTS_POSITION_PATH_BEGIN, dir);
