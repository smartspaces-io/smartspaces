...
package io.smartspaces.bridge.message.ros;
...
public class OgnlRosMessageBridgeSpecificationCompiler implements
  ...
  @Override
  public <SourceType, DestinationType> RosMessageBridgeSpecification<SourceType, DestinationType>
      compile(String source) {
    String[] lines = source.split(";");
    if (lines.length < 2) {
      throw new SmartSpacesException("Not enough lines in the bridge specification");
    }

    String[] sources = lines[0].split(":");
    if (sources.length != 2) {
      throw new SmartSpacesException(String.format("Illegal message bridge source %s", lines[0]));
    }

    String[] destinations = lines[1].split(":");
    if (destinations.length != 2) {
      throw new SmartSpacesException(String.format("Illegal message bridge destination %s",
          lines[1]));
    }
    ...
    return new OgnlRosMessageBridgeSpecification<SourceType, DestinationType>(sourceTopicName,
        sourceMessageType, destinationTopicName, destinationMessageType, expressions);
  }
}
