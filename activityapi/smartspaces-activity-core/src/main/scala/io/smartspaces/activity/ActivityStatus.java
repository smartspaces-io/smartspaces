...
public class ActivityStatus {
  ...
  public String getExceptionAsString() {
    if (exception != null) {
      if (exception instanceof SimpleSmartSpacesException) {
        return String.format("Exception message: %s",
            ((SimpleSmartSpacesException) exception).getCompoundMessage());
      } else {
        return SmartSpacesException.getStackTrace(exception);
      }
    } else {
      return null;
    }
  ...
  /**
   * Get the combined detail of the description, if any, and the exception as a
   * string, if any.
   *
