 ...
 */
public class SimplePropertyFileSingleConfigurationStorageManager implements
   ...
   *          the file containing the configuration
   */
  public SimplePropertyFileSingleConfigurationStorageManager(boolean required,
    ...
    if (configurationFile.exists()) {
      ...
      if (newLastModified > lastModifiedTime) {
        ...
        try {
          ...
          throw new SmartSpacesException(String.format("Cannot read configuration file %s",
      ...
      if (required) {
        throw new SimpleSmartSpacesException(String.format(
            "Cannot locate required configuration file %s", configurationFile.getAbsolutePath()));
      }
    }
  ...
  public SingleConfigurationStorageManager save() {
    ...
    try {
      ...
      throw new SmartSpacesException("Could not save configuration file " + configurationFile, e);
    } finally {
      if (out != null) {
        try {
          out.close();
