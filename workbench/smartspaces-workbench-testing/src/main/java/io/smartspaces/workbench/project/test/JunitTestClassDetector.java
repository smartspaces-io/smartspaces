...
public class JunitTestClassDetector {
   ...
   *          logger for the test run
   */
  private void scanDirectory(File directory, List<Class<?>> testClasses,
      StringBuilder packagePrefix, ClassLoader classLoader, Log log) {
   ...
   *          the logger to use
   */
  private void testClassForTests(StringBuilder packagePrefix, String classFileName,
      ClassLoader classLoader, List<Class<?>> testClasses, Log log) {
    ...
    fullyQualifiedClassName.append(classFileName.substring(0,
        classFileName.indexOf(FILE_EXTENSION_SEPARATOR)));
    try {
  ...
  private boolean classHasPublicConstructor(Class<?> clazz, Log log) {
    try {
      ...
      log.warn(
          String.format("Potential test class %s caused a security exception. Ignoring.",
              clazz.getName()), e);
