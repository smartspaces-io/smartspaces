 ...
 * tests that are common for all types. Subclasses can add specific tests.
 * <p>
 * State of a dynamic object can be queried and modified in two ways - through
 * the backing map and via setters and getters of the dynamic object. This class
 * tests the basic sequences of setting and getting values both ways.
 *
 * @param <T>
...
public abstract class BaseDynamicPropertyTest<T> {
  /**
   * Map that contains state of a dynamic object. ConcurrentSkipListMap is used
   * because it does not allow null keys or values, and it will throw an
   * exception (ClassCastException) if DynamicObject implementation tries to put
   * there something other than String.
   */
  ...
  /**
   * Key in the backing map that corresponds to the JavaBean property name for
   * the first value.
   */
