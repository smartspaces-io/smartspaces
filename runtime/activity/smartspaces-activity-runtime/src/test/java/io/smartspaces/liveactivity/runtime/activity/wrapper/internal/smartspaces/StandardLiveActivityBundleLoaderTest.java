...
public class StandardLiveActivityBundleLoaderTest {
  ...
  public void testLoad() throws Exception {
    ...
    when(containerResourceManager.loadAndStartBundle(bundleFile, ContainerResourceType.ACTIVITY))
        .thenReturn(bundle);
  ...
  public void testLoadExceptionStart() throws Exception {
    ...
    when(containerResourceManager.loadAndStartBundle(bundleFile, ContainerResourceType.ACTIVITY))
        .thenThrow(new SmartSpacesException("foo"));
  ...
  /**
   * Load an activity bundle then loading another bundle for the same activity
  ...
  public void testLoad2() throws Exception {
    ...
    when(containerResourceManager.loadAndStartBundle(bundleFile1, ContainerResourceType.ACTIVITY))
        .thenReturn(bundle1);
    when(containerResourceManager.loadAndStartBundle(bundleFile2, ContainerResourceType.ACTIVITY))
        .thenReturn(bundle2);
    ...
    verify(containerResourceManager, times(0)).loadAndStartBundle(bundleFile2,
        ContainerResourceType.ACTIVITY);
  }

  /**
   * Load an activity bundle then loading another bundle for the same activity
   * with the same signature. Then unload. The bundle should not be unloaded at
   * all.
   */
  @Test
