...
class PlaybackRunner implements Runnable {
   ...
   *          {@code true} if messages should be played back on a route
   */
  public PlaybackRunner(StandaloneMessageRouter standaloneMessageRouter, File playbackFile,
      boolean onRoute) {
    ...
    try {
      ...
      throw new SmartSpacesException("Could not create playback runner", e);
    }
  }
  ...
  public void run() {
    try {
        ...
        try {
          ...
          if (!(timeDelay instanceof Integer)) {
            throw new SimpleSmartSpacesException(String.format(
                "Message must have an Integer field '%s': %s",
                StandaloneMessageRouter.TIME_DELAY_KEY, messageObject));
          }
          long deltaTime = (Integer) timeDelay;
          standaloneMessageRouter.getLog().debug(
              String.format("Playback next message in %dms...", deltaTime));
      ...
      standaloneMessageRouter.handleError(
          "While processing playback file " + playbackFile.getAbsolutePath(), e);
    }
  }
