 ...
 */
public class StandardSpaceControllerDataBundleManager implements SpaceControllerDataBundleManager {
   ...
   */
  public StandardSpaceControllerDataBundleManager() {
   ...
   */
  public StandardSpaceControllerDataBundleManager(HttpContentCopier contentCopier,
  ...
  private File createDataBundle() {
    ...
    try {
      ...
      addDataBundleSection(zipOutputStream, CONTROLLER_DATA_BUNDLE_ENTRY,
      ...
      throw new SmartSpacesException(String.format("Error while zipping data bundle %s",
   ...
   */
  private void addDataBundleSection(ZipOutputStream zipOutputStream, String subsection,
    ...
    if (contentDirectory.exists()) {
      ...
      getLog().warn(
          String.format("Skipping non-existent data directory %s",
              contentDirectory.getAbsolutePath()));
    }
  }
  ...
  private void extractFullDataBundle(File dataBundle) {
    ...
    try {
      ...
      for (InstalledLiveActivity activity : spaceController.getAllInstalledLiveActivities()) {
        extractDataBundleSection(incomingDirectory,
   ...
   */
  private void extractDataBundleSection(File incomingDirectory, String subsection,
    ...
    try {
      if (targetDirectory.exists() && !fileSupport.rename(targetDirectory, backupDirectory)) {
        throw new SimpleSmartSpacesException("Could not move existing directory to backup");
      }
      if (!sourceDirectory.exists()) {
        getLog().warn(String.format("No source directory found for %s, skipping", sourceDirectory));
      } else {
        getLog().info(
            String.format("Extracting source %s to %s, backup %s", sourceDirectory,
                targetDirectory, backupDirectory));
        if (!fileSupport.rename(sourceDirectory, targetDirectory)) {
          throw new SimpleSmartSpacesException("Could not move incoming directory to target");
        }
      }
      ...
      throw new SmartSpacesException("Renaming content directories for "
  ...
  @Override
  public void setActivityStorageManager(LiveActivityStorageManager activityStorageManager) {
  ...
  private File getActivityDataContentDirectory(InstalledLiveActivity activity) {
    return activityStorageManager.getActivityFilesystem(activity.getUuid())
